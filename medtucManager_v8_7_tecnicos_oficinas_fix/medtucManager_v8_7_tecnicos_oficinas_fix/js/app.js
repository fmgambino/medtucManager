...
function mapImport(key,x){
  ...
  return {
    ...
    fault_description:get('Falla','Problema','Descripcion','Descripción','FallaDeclarada','fault_description') || 'Importado desde SATMANAGER',
...
/* ========================= v8.6 requested fixes =========================
   ...
   - orden de servicio con más datos SATMANAGER
============================================================================ */
...
function mapImport(key,x){
  ...
  const obs = [technician?`Técnico SATMANAGER: ${technician}`:'', obsBase].filter(Boolean).join(' | ');
  return {
    ...
    fault_description:satGet(r,'Falla','ReparacionesFalla','Problema','Descripcion','Descripción','FallaDeclarada','fault_description') || 'Importado desde SATMANAGER',
...
function orderRow(o){
  ...
  const tech=o.technician_name || (String(o.observations||'').match(/Técnico SATMANAGER:\s*([^|]+)/)||[])[1] || '';
...
window.viewOrder = async(id)=>{
  ...
  const html=`<div class="detail-card order-detail"><h3>Orden #${esc(o.satmanager_order||o.order_number)}</h3><div class="detail-grid"><p><b>Ingreso original</b><span>${fmt(o.received_at)}</span></p><p><b>Estado</b><span>${esc(o.service_order_statuses?.name||'Sin estado')}</span></p><p><b>Oficina/Repartición</b><span>${esc(o.office||o.requester_name||'-')}</span></p><p><b>Solicitante</b><span>${esc(o.requester_name||'-')}</span></p><p><b>Técnico SATMANAGER</b><span>${esc(tech)}</span></p><p><b>Dirección</b><span>${esc(o.address||'-')}</span></p><p><b>Equipo</b><span>${esc([o.equipment_type,o.brand,o.model].filter(Boolean).join(' · ')||'-')}</span></p><p><b>N° serie</b><span>${esc(o.serial_number||'-')}</span></p><p><b>Accesorios</b><span>${esc(o.accessories||'-')}</span></p><p><b>Tabla origen</b><span>${esc(o.satmanager_table||'REPARACIONES')}</span></p><p class="full"><b>Falla</b><span>${esc(o.fault_description||'-')}</span></p><p class="full"><b>Informe técnico</b><span>${esc(o.technical_report||'-')}</span></p><p class="full"><b>Solución / Observaciones</b><span>${esc(o.solution||o.observations||'-')}</span></p></div><h4>Historial</h4>${(hist||[]).map(h=>`<p class="history-line"><b>${fmt(h.created_at)}</b> · ${esc(h.action)} ${h.previous_value?`<small>${esc(h.previous_value)} → ${esc(h.new_value||'')}</small>`:''}</p>`).join('') || '<p>Sin movimientos registrados.</p>'}</div>`;
...
window.openOrder = async(id=null)=>{
  ...
  const html=`<div class="tm-form order-form"><div class="tm-form-header"><div class="tm-form-avatar">${ico.orders}</div><p>${id?'Actualizar trazabilidad técnica, estado e informe de la orden.':'Registrar una nueva orden técnica institucional.'}</p></div><div class="swal-grid">${statusSelect}${inputHtml('satmanager_order','Nº Orden / SATMANAGER',o?.satmanager_order)}${inputHtml('technician_name','Técnico que atiende',o?.technician_name)}${inputHtml('requester_name','Solicitante / Cliente',o?.requester_name)}${inputHtml('office','Oficina/Repartición',o?.office)}${inputHtml('requester_phone','Teléfono',o?.requester_phone)}${inputHtml('requester_email','Email',o?.requester_email)}${inputHtml('address','Dirección',o?.address)}${inputHtml('equipment_type','Tipo de equipo',o?.equipment_type)}${inputHtml('brand','Marca',o?.brand)}${inputHtml('model','Modelo',o?.model)}${inputHtml('serial_number','N° de serie',o?.serial_number)}${inputHtml('priority','Prioridad',o?.priority||'Media')}${inputHtml('accessories','Accesorios',o?.accessories)}${inputHtml('fault_description','Falla declarada',o?.fault_description)}${inputHtml('technical_report','Informe técnico',o?.technical_report)}${inputHtml('solution','Solución / Observaciones',o?.solution)}</div></div>`;
...
function extractTechFromOrder(o){ return o?.technician_name || o?.professional_technician || (String(o?.observations||'').match(/Técnico SATMANAGER:\s*([^|]+)/i)||[])[1] || ''; }
...
window.syncTechniciansAndOffices = async () => {
  const wait = Swal.fire({title:'Actualizando usuarios técnicos',html:'Sincronizando profesionales técnicos y oficinas/reparticiones desde SATMANAGER...',allowOutsideClick:false,didOpen:()=>Swal.showLoading()});
  try{
...
window.openOrder = async(id=null)=>{
  ...
  const o=id?(state.rows.orders||[]).find(x=>x.id===id):{};
  const statusSelect=`<label>Estado<select id="f_status_id"><option value="">Sin estado</option>${state.statuses.map(s=>`<option value="${s.id}" ${o?.status_id===s.id?'selected':''}>${esc(s.name)}</option>`).join('')}</select></label>`;
  const html=`<div class="tm-form order-form"><div class="tm-form-header"><div class="tm-form-avatar">${ico.orders}</div><p>${id?'Actualizar trazabilidad técnica, estado e informe de la orden.':'Registrar una nueva orden técnica institucional.'}</p></div><div class="swal-grid">${statusSelect}${inputHtml('satmanager_order','Nº Orden / SATMANAGER',o?.satmanager_order)}${technicianSelectHtml(extractTechFromOrder(o))}${inputHtml('requester_name','Solicitante / Referente',o?.requester_name)}${officeSelectHtml(o?.office)}${inputHtml('requester_phone','Teléfono',o?.requester_phone)}${inputHtml('requester_email','Email',o?.requester_email)}${inputHtml('address','Dirección',o?.address)}${inputHtml('equipment_type','Tipo de equipo',o?.equipment_type)}${inputHtml('brand','Marca',o?.brand)}${inputHtml('model','Modelo',o?.model)}${inputHtml('serial_number','N° de serie',o?.serial_number)}${inputHtml('priority','Prioridad',o?.priority||'Media')}${inputHtml('accessories','Accesorios / Insumos asociados',o?.accessories)}${inputHtml('fault_description','Falla / Diagnóstico',o?.fault_description)}${inputHtml('technical_report','Informe técnico',o?.technical_report)}${inputHtml('solution','Solución / Observaciones',o?.solution||o?.observations)}</div></div>`;
...
function renderShell(){
  ...
  $('#fullBtn')?.addEventListener('click', () => document.fullscreenElement ? document.exitFullscreen() : document.documentElement.requestFullscreen());
  $('#notifyBtn').onclick = () => route('notifications');
  $('#notifyBtn').onmouseenter = showNotifyPreview;
