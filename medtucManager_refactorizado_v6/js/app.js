...
(function(){
  ...
  // v6 institucional: perfiles oficiales, módulo de Órdenes de Servicio sin importes y trazabilidad.
  window.tmRole = function(role){
    const raw = String(role || state.user?.role || 'Usuario').trim().toLowerCase();
    if (['superadmin','super admin','owner','root'].includes(raw)) return 'SuperAdmin';
    if (['admin','administrador','administrator'].includes(raw)) return 'Admin';
    if (['tecnico','técnico','technician','tecnicos','técnicos'].includes(raw)) return 'Tecnico';
    return 'Usuario';
  };
  state.user = Object.assign({name:'Ing. Fernando Gambino', email:'fernando.m.gambino@gmail.com', role:'SuperAdmin'}, state.user || {});
  if (state.user.email === 'admin@ism.edu.ar') state.user.email = 'fernando.m.gambino@gmail.com';
  if (!['SuperAdmin','Admin','Tecnico','Usuario'].includes(window.tmRole(state.user.role))) state.user.role = 'SuperAdmin';

  state.rolesData = [
    { id:'SuperAdmin', code:'SuperAdmin', name:'SuperAdmin', description:'Acceso institucional completo, configuración de roles, usuarios, módulos, auditoría e integraciones.', permissions:['*'] },
    { id:'Admin', code:'Admin', name:'Admin', description:'Administración operativa de tickets, órdenes, inventario, préstamos, usuarios y reportes.', permissions:['dashboard.read','users.manage','roles.read','orders.manage','inventory.manage','loans.manage','tickets.manage','notifications.send','settings.read'] },
    { id:'Tecnico', code:'Tecnico', name:'Técnicos', description:'Atención técnica, actualización de estados, informes, uso de insumos y seguimiento de tickets asignados.', permissions:['dashboard.read','orders.read','orders.manage_assigned','inventory.read','inventory.consume','tickets.read','tickets.manage_assigned','notifications.read','profile.manage'] },
    { id:'Usuario', code:'Usuario', name:'Usuarios', description:'Oficinas y reparticiones. Carga y consulta de tickets propios, notificaciones y perfil.', permissions:['dashboard.read','tickets.create','tickets.read_own','notifications.read','profile.manage'] }
  ];

  const officialViews = {
    SuperAdmin:['dashboard','users','roles','serviceOrders','inventory','loanManagement','supportTickets','notifications','profile','settings'],
    Admin:['dashboard','users','roles','serviceOrders','inventory','loanManagement','supportTickets','notifications','profile','settings'],
    Tecnico:['dashboard','serviceOrders','inventory','loanManagement','supportTickets','notifications','profile'],
    Usuario:['dashboard','supportTickets','notifications','profile']
  };
  const officialLabels = {
    dashboard:['Dashboard','Vista ejecutiva de trazabilidad técnica institucional.'],
    users:['Usuarios','Administración de usuarios institucionales, perfiles y estado de acceso.'],
    roles:['Roles y Permisos','Matriz configurable de permisos por perfil: SuperAdmin, Admin, Técnicos y Usuarios.'],
    serviceOrders:['Órdenes de Servicio','Carga, atención, estados, historia, insumos, notas e impresiones técnicas.'],
    inventory:['Inventario','Control de activos e insumos con stock, préstamos, consumo y trazabilidad.'],
    loanManagement:['Gestión de Préstamos','Solicitudes, aprobaciones, devoluciones y seguimiento operativo.'],
    supportTickets:['Soporte Ticket','Tickets generados desde formulario público y circuito interno de asistencia técnica.'],
    notifications:['Notificaciones','Alertas operativas, atención de órdenes/tickets y cambios de estado.'],
    profile:['Mi Perfil','Datos personales, foto y preferencias de usuario.'],
    settings:['Configuraciones','Parámetros institucionales, logos, Supabase, Telegram y paleta visual.']
  };
  getVisibleViews = function(){ return officialViews[window.tmRole(state.user?.role)] || officialViews.Usuario; };
  getRoleLabels = function(){ return officialLabels; };
  roleLabelFromAny = function(role){ return window.tmRole(role); };
  canManageTickets = function(){ return ['SuperAdmin','Admin','Tecnico'].includes(window.tmRole(state.user?.role)); };

  Object.assign(icons, {
    serviceOrders:`<svg viewBox="0 0 24 24" class="icon"><path d="M7 2h10a2 2 0 0 1 2 2v16.5l-2.8-1.8-2.8 1.8-2.8-1.8-2.8 1.8L5 18.7V4a2 2 0 0 1 2-2Zm2 5h6v2H9V7Zm0 4h6v2H9v-2Zm0 4h4v2H9v-2Z"/></svg>`,
    note:`<svg viewBox="0 0 24 24" class="icon"><path d="M4 3h12l4 4v14H4V3Zm11 1.5V8h3.5L15 4.5ZM7 11h10v2H7v-2Zm0 4h10v2H7v-2Z"/></svg>`,
    ai:`<svg viewBox="0 0 24 24" class="icon"><path d="M12 2l1.2 4.2L17 8l-3.8 1.8L12 14l-1.2-4.2L7 8l3.8-1.8L12 2Zm6 8 1 3 3 1-3 1-1 3-1-3-3-1 3-1 1-3ZM6 13l.8 2.2L9 16l-2.2.8L6 19l-.8-2.2L3 16l2.2-.8L6 13Z"/></svg>`,
    pdf:`<svg viewBox="0 0 24 24" class="icon"><path d="M6 2h9l5 5v15H6a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2Zm8 1.5V8h4.5L14 3.5ZM7 12h10v2H7v-2Zm0 4h7v2H7v-2Z"/></svg>`
  });

  state.orderStatuses = JSON.parse(localStorage.getItem('tm_order_statuses') || 'null') || [
    {name:'Pendiente', color:'#f59e0b'}, {name:'En proceso', color:'#3b82f6'}, {name:'Terminada', color:'#22c55e'}, {name:'Lista p/Retirar', color:'#8b5cf6'}, {name:'Entregada', color:'#14b8a6'}, {name:'Cancelada', color:'#ef4444'}
  ];
  state.serviceOrders = JSON.parse(localStorage.getItem('tm_service_orders') || 'null') || [
    {id:22, order:22, date:'2025-10-02', time:'15:54', client:'Presupuesto', address:'Av. Sarmiento 850', phone:'', technician:'Mateo Fornaciari', serial:'88', type:'PC', brand:'MAGNUM', model:'MAGNUM', accessories:'', failure:'Unidad de estado sólido con falla crítica; se solicitará reemplazo.', report:'', status:'Pendiente', source_ticket:'TK-2026-00001', usedItems:[], history:[{date:new Date().toLocaleString('es-AR'), user:'Sistema', action:'Orden importada desde SATMANAGER'}]},
    {id:31, order:31, date:'2025-10-27', time:'09:20', client:'Novedades Salariales', address:'', phone:'', technician:'', serial:'', type:'Notebook', brand:'Clon', model:'S/M', accessories:'Cargador', failure:'No funciona. Revisar Pregase.', report:'', status:'En proceso', source_ticket:'', usedItems:[], history:[]},
    {id:90, order:90, date:'2026-03-05', time:'10:10', client:'Infraestructura Escolar', address:'', phone:'', technician:'', serial:'', type:'PC', brand:'Magnum Tech', model:'MTK835', accessories:'', failure:'Inicio de Windows dañado.', report:'', status:'Terminada', source_ticket:'', usedItems:[], history:[]}
  ].map(o => { delete o.labor; delete o.parts; delete o.advance; delete o.balance; return o; });
  state.serviceOrderFilter = state.serviceOrderFilter || 'Todas';
  ...
  function persistStatuses(){ localStorage.setItem('tm_order_statuses', JSON.stringify(state.orderStatuses)); }
  function notify(title, message){
    state.notifications = state.notifications || [];
    state.notifications.unshift({id:Date.now()+Math.random(), title, message, unread:true, section:'serviceOrders', createdAt:new Date().toLocaleString('es-AR')});
    try { localStorage.setItem('tm_notifications', JSON.stringify(state.notifications)); } catch(e) {}
    if (typeof renderNotificationDropdown === 'function') renderNotificationDropdown();
  }
  function currentUserLabel(){ return `${state.user?.name || 'Usuario'} (${window.tmRole(state.user?.role)})`; }
  function addHistory(o, action){
    o.history = o.history || [];
    const stamp = new Date().toLocaleString('es-AR');
    o.history.unshift({date:stamp, user:currentUserLabel(), action});
    notify('Trazabilidad de Orden de Servicio', `Orden N° ${o.order}: ${action}. Registro: ${stamp}. Usuario: ${currentUserLabel()}.`);
  }
  function statusColor(name){ return (state.orderStatuses||[]).find(s=>s.name===name)?.color || '#64748b'; }
  function orderStatusPill(s){ const c=statusColor(s); return `<span class="status-pill" style="background:${c}22;color:${c};border-color:${c}66">${escapeHtml(s||'Sin estado')}</span>`; }
  function filteredOrders(){
    const q=String(state.serviceOrderSearch||'').toLowerCase();
    return state.serviceOrders.filter(o => (state.serviceOrderFilter==='Todas'||o.status===state.serviceOrderFilter) && [o.order,o.date,o.client,o.technician,o.serial,o.type,o.brand,o.model,o.failure,o.report,o.source_ticket].some(v=>String(v||'').toLowerCase().includes(q)));
  }
  function nextOrderNumber(){ return Math.max(0,...state.serviceOrders.map(o=>Number(o.order)||0))+1; }

  window.renderServiceOrders = function(){
    const rows = filteredOrders();
    const statusFilters = ['Todas', ...state.orderStatuses.map(s=>s.name)];
    const canEdit = ['SuperAdmin','Admin','Tecnico'].includes(window.tmRole(state.user?.role));
    return `<section class="tm-module-head"><div><h2>Órdenes de Servicio</h2><p>Gestión de asistencia técnica institucional: ingreso, atención, historia, consumo de insumos, impresión y entrega.</p></div><div class="toolbar"><button class="btn btn-primary btn-sm" id="newServiceOrderBtn">${icons.serviceOrders} Nueva orden</button><button class="btn btn-secondary btn-sm" id="importServiceOrdersBtn">${icons.import} Importar SATMANAGER</button><button class="btn btn-secondary btn-sm" data-export="serviceOrders">${icons.export} CSV</button><button class="btn btn-secondary btn-sm" data-pdf-export="serviceOrders">${icons.pdf} PDF A4</button></div></section>
    <section class="sat-panel professional-orders"><article class="card glass"><div class="section-header"><div><h3>Panel operativo</h3><p class="muted">Inspirado en SATMANAGER, adaptado al Ministerio de Educación Tucumán sin importes ni información comercial.</p></div><button class="btn btn-secondary btn-sm" id="manageStatusesBtn">Configurar estados</button></div>
    <div class="sat-toolbar"><input id="serviceOrderSearch" type="search" placeholder="Buscar por orden, repartición, técnico, serie, equipo, falla o ticket" value="${escapeAttr(state.serviceOrderSearch)}"><div class="sat-status">${statusFilters.map(s=>`<label class="sat-radio"><input type="radio" name="soStatus" value="${escapeAttr(s)}" ${state.serviceOrderFilter===s?'checked':''}> ${escapeHtml(s)}</label>`).join('')}</div></div>
    <div class="order-kpis"><div><strong>${state.serviceOrders.length}</strong><span>Total</span></div>${state.orderStatuses.slice(0,4).map(st=>`<div><strong>${state.serviceOrders.filter(o=>o.status===st.name).length}</strong><span>${escapeHtml(st.name)}</span></div>`).join('')}</div>
    <div class="table-wrap"><table class="table professional-table"><thead><tr><th>Orden</th><th>Ingreso</th><th>Repartición / Oficina</th><th>Técnico</th><th>Equipo</th><th>Serie</th><th>Falla declarada</th><th>Estado</th><th>Acciones</th></tr></thead><tbody>${rows.map(o=>`<tr><td><strong>OS-${String(o.order).padStart(5,'0')}</strong><small>${escapeHtml(o.source_ticket||'')}</small></td><td>${escapeHtml(formatDateOnlyAR(o.date))}<small>${escapeHtml(o.time||'')}</small></td><td>${escapeHtml(o.client||'-')}</td><td>${escapeHtml(o.technician||'Sin asignar')}</td><td>${escapeHtml([o.type,o.brand,o.model].filter(Boolean).join(' · ')||'-')}</td><td>${escapeHtml(o.serial||'-')}</td><td>${escapeHtml(o.failure||'-')}</td><td>${orderStatusPill(o.status)}</td><td><div class="action-group compact"><button class="action-btn" title="Ver" data-so-view="${o.id}">${icons.eye}</button>${canEdit?`<button class="action-btn" title="Editar" data-so-edit="${o.id}">${icons.edit}</button><button class="action-btn" title="Cambiar estado" data-so-status="${o.id}">${icons.finish||icons.approve}</button><button class="action-btn" title="Nota/IA" data-so-note="${o.id}">${icons.note}</button><button class="action-btn" title="Usar insumo" data-so-consume="${o.id}">${icons.inventory}</button>`:''}<button class="action-btn" title="Imprimir ingreso" data-so-print-in="${o.id}">${icons.print||icons.pdf}</button><button class="action-btn" title="Imprimir entrega" data-so-print-out="${o.id}">${icons.returnIcon}</button><button class="action-btn" title="Historia" data-so-history="${o.id}">${icons.history||icons.notifications}</button>${canEdit?`<button class="action-btn danger" title="Eliminar" data-so-delete="${o.id}">${icons.trash}</button>`:''}</div></td></tr>`).join('')||'<tr><td colspan="9" class="muted">Sin órdenes registradas.</td></tr>'}</tbody></table></div></article>
    <aside class="card glass"><h3>Centro de acciones</h3><p class="muted">Convierta tickets en órdenes técnicas, configure estados y genere documentación institucional.</p><div class="stack"><button class="btn btn-primary full-width" id="generateOrderFromTicketBtn">Crear orden desde ticket pendiente</button><button class="btn btn-secondary full-width" id="newServiceOrderNoteBtn">Redactar nota institucional</button><button class="btn btn-secondary full-width" id="smartAccessImportHelpBtn">Guía de importación Access</button></div></aside></section>`;
  };

  async function openServiceOrderForm(order){
    const next = nextOrderNumber();
    const statusOptions = state.orderStatuses.map(s=>`<option value="${escapeAttr(s.name)}" ${order?.status===s.name?'selected':''}>${escapeHtml(s.name)}</option>`).join('');
    const html = `<div class="swal-form-grid order-form-v6">
      <label><span>Orden</span><input id="so_order" class="swal2-input" value="${escapeAttr(order?.order||next)}"></label>
      <label><span>Fecha de ingreso</span><input id="so_date" type="date" class="swal2-input" value="${escapeAttr((order?.date||new Date().toISOString()).slice(0,10))}"></label>
      <label><span>Hora</span><input id="so_time" type="time" class="swal2-input" value="${escapeAttr(order?.time||new Date().toTimeString().slice(0,5))}"></label>
      <label><span>Técnico asignado</span><input id="so_technician" class="swal2-input" value="${escapeAttr(order?.technician||'')}"></label>
      <label><span>Repartición / oficina</span><input id="so_client" class="swal2-input" value="${escapeAttr(order?.client||'')}"></label>
      <label><span>Dirección</span><input id="so_address" class="swal2-input" value="${escapeAttr(order?.address||'')}"></label>
      <label><span>Teléfono / interno</span><input id="so_phone" class="swal2-input" value="${escapeAttr(order?.phone||'')}"></label>
      <label><span>Ticket origen</span><input id="so_source_ticket" class="swal2-input" value="${escapeAttr(order?.source_ticket||'')}"></label>
      <label><span>Número de serie</span><input id="so_serial" class="swal2-input" value="${escapeAttr(order?.serial||'')}"></label>
      <label><span>Tipo de equipo</span><input id="so_type" class="swal2-input" value="${escapeAttr(order?.type||'PC')}"></label>
      <label><span>Marca</span><input id="so_brand" class="swal2-input" value="${escapeAttr(order?.brand||'')}"></label>
      <label><span>Modelo</span><input id="so_model" class="swal2-input" value="${escapeAttr(order?.model||'')}"></label>
      <label><span>Estado</span><select id="so_status" class="swal2-select">${statusOptions}</select></label>
      <label class="full-span"><span>Accesorios recibidos</span><textarea id="so_accessories" class="swal2-textarea">${escapeHtml(order?.accessories||'')}</textarea></label>
      <label class="full-span"><span>Falla declarada</span><textarea id="so_failure" class="swal2-textarea">${escapeHtml(order?.failure||'')}</textarea></label>
      <label class="full-span"><span>Informe técnico / intervención realizada</span><textarea id="so_report" class="swal2-textarea">${escapeHtml(order?.report||'')}</textarea></label>
    </div>`;
    const res = await Swal.fire({title: order?'Editar Orden de Servicio':'Nueva Orden de Servicio', width:960, html, showCancelButton:true, confirmButtonText:'Guardar trazabilidad', cancelButtonText:'Cancelar', preConfirm:()=>({id:order?.id||Date.now(), order:document.getElementById('so_order').value, date:document.getElementById('so_date').value, time:document.getElementById('so_time').value, technician:document.getElementById('so_technician').value, client:document.getElementById('so_client').value, address:document.getElementById('so_address').value, phone:document.getElementById('so_phone').value, source_ticket:document.getElementById('so_source_ticket').value, serial:document.getElementById('so_serial').value, type:document.getElementById('so_type').value, brand:document.getElementById('so_brand').value, model:document.getElementById('so_model').value, accessories:document.getElementById('so_accessories').value, failure:document.getElementById('so_failure').value, report:document.getElementById('so_report').value, status:document.getElementById('so_status').value, usedItems:order?.usedItems||[], history:order?.history||[]})});
    if (!res.isConfirmed) return;
    if (order) { const before = order.status; Object.assign(order, res.value); addHistory(order, before!==order.status ? `Cambio de estado: ${before} → ${order.status}` : 'Actualización de datos de la orden'); }
    else { state.serviceOrders.unshift(res.value); addHistory(res.value, 'Alta de Orden de Servicio'); }
    persistOrders(); renderView(); showToast('Orden guardada','La orden fue actualizada correctamente.');
  }

  async function changeOrderStatus(o){
    const statusOptions = state.orderStatuses.map(s=>`<option value="${escapeAttr(s.name)}" ${o.status===s.name?'selected':''}>${escapeHtml(s.name)}</option>`).join('');
    const res = await Swal.fire({title:'Cambiar estado de la orden', html:`<label class="swal-label">Estado operativo</label><select id="new_so_status" class="swal2-select">${statusOptions}</select><label class="swal-label">Observación</label><textarea id="new_so_obs" class="swal2-textarea" placeholder="Detalle del cambio de estado"></textarea>`, showCancelButton:true, confirmButtonText:'Actualizar estado', preConfirm:()=>({status:document.getElementById('new_so_status').value, obs:document.getElementById('new_so_obs').value})});
    if(!res.isConfirmed) return;
    const old=o.status; o.status=res.value.status; addHistory(o, `Cambio de estado: ${old} → ${o.status}${res.value.obs?' · '+res.value.obs:''}`); persistOrders(); renderView();
  }

  async function manageStatuses(){
    const list = state.orderStatuses.map((s,i)=>`<div class="status-editor-row"><input data-status-name="${i}" class="swal2-input" value="${escapeAttr(s.name)}"><input data-status-color="${i}" type="color" class="swal2-input color-mini" value="${escapeAttr(s.color)}"><button type="button" class="btn btn-secondary btn-sm" data-remove-status="${i}">Quitar</button></div>`).join('');
    const res = await Swal.fire({title:'Estados de Órdenes de Servicio', width:760, html:`<div id="statusEditor">${list}</div><button type="button" id="addStatusRow" class="btn btn-primary btn-sm">Agregar nuevo estado</button>`, showCancelButton:true, confirmButtonText:'Guardar estados', didOpen:()=>{document.getElementById('addStatusRow').onclick=()=>{const wrap=document.getElementById('statusEditor'); const i=wrap.children.length; wrap.insertAdjacentHTML('beforeend',`<div class="status-editor-row"><input data-status-name="${i}" class="swal2-input" value="Nuevo estado"><input data-status-color="${i}" type="color" class="swal2-input color-mini" value="#64748b"><button type="button" class="btn btn-secondary btn-sm" onclick="this.parentElement.remove()">Quitar</button></div>`)}; document.querySelectorAll('[data-remove-status]').forEach(b=>b.onclick=()=>b.parentElement.remove());}, preConfirm:()=>Array.from(document.querySelectorAll('#statusEditor .status-editor-row')).map(row=>({name:row.querySelector('[data-status-name]')?.value.trim(), color:row.querySelector('[data-status-color]')?.value||'#64748b'})).filter(s=>s.name)});
    if(res.isConfirmed){ state.orderStatuses=res.value; persistStatuses(); renderView(); }
  }

  function printOrder(o, type='ingreso'){
    const isOut = type==='entrega';
    const title = isOut ? 'Constancia de Entrega de Equipo' : 'Constancia de Ingreso de Equipo';
    const logo = document.body.classList.contains('light') ? 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQ4m5sI9rXqK72J4Ix5rqSA4U4AH3eDjmuJWQ&s' : 'https://www.educaciontuc.gov.ar/wp-content/uploads/2024/10/MINISTERIO-DE-EDUCACION-blanco.png';
    const now = new Date().toLocaleString('es-AR');
    const body = `<html><head><title>${title} OS-${o.order}</title><style>body{font-family:Arial,sans-serif;margin:0;background:#e5e7eb}.sheet{width:190mm;min-height:270mm;margin:12mm auto;background:#fff;color:#111;padding:12mm;box-shadow:0 0 0 1px #ddd}.rotulo{display:flex;align-items:center;gap:16px;border:2px solid #111;padding:10px;margin-bottom:14px}.rotulo img{max-width:165px;max-height:60px;object-fit:contain;background:#111;padding:4px}.rotulo h1{font-size:20px;margin:0}.meta{font-size:12px}.box{border:1px solid #111;margin:10px 0;padding:10px}.grid{display:grid;grid-template-columns:1fr 1fr;gap:8px}.sign{display:grid;grid-template-columns:1fr 1fr;gap:40px;margin-top:45px}.line{border-top:1px solid #111;text-align:center;padding-top:6px}.small{font-size:12px;color:#333}@media print{body{background:#fff}.sheet{box-shadow:none;margin:0;width:auto}}</style></head><body><main class="sheet"><div class="rotulo"><img src="${logo}"><div><h1>Área Técnica - Dirección de Informática</h1><strong>${title}</strong><div class="meta">Orden: OS-${String(o.order).padStart(5,'0')} · Reporte generado: ${now}</div></div></div><div class="box"><strong>Ingreso:</strong> ${formatDateOnlyAR(o.date)} ${o.time||''} · <strong>Estado:</strong> ${o.status||'-'} · <strong>Técnico:</strong> ${o.technician||'Sin asignar'}</div><div class="box"><h3>Repartición / solicitante</h3><div class="grid"><div><strong>Oficina:</strong> ${escapeHtml(o.client||'-')}</div><div><strong>Teléfono:</strong> ${escapeHtml(o.phone||'-')}</div><div><strong>Dirección:</strong> ${escapeHtml(o.address||'-')}</div><div><strong>Ticket origen:</strong> ${escapeHtml(o.source_ticket||'-')}</div></div></div><div class="box"><h3>Equipo</h3><div class="grid"><div><strong>Tipo:</strong> ${escapeHtml(o.type||'-')}</div><div><strong>Serie:</strong> ${escapeHtml(o.serial||'-')}</div><div><strong>Marca:</strong> ${escapeHtml(o.brand||'-')}</div><div><strong>Modelo:</strong> ${escapeHtml(o.model||'-')}</div></div><p><strong>Accesorios:</strong> ${escapeHtml(o.accessories||'-')}</p><p><strong>Falla declarada:</strong> ${escapeHtml(o.failure||'-')}</p>${isOut?`<p><strong>Informe técnico:</strong> ${escapeHtml(o.report||'-')}</p>`:''}</div><div class="box small">Sin valorización económica. Documento interno para trazabilidad técnica institucional.</div><div class="sign"><div class="line">Firma y aclaración área solicitante</div><div class="line">Firma y aclaración área técnica</div></div></main><script>window.onload=()=>print()<\/script></body></html>`;
    const w=window.open('','_blank'); w.document.write(body); w.document.close();
  }

  async function consumeInventory(o){
    const insumos = (state.inventory||[]).filter(i => String(i.type||'').toLowerCase().includes('insumo') || Number(i.stock||0)>0 || i.status==='Disponible');
    if(!insumos.length) return Swal.fire({icon:'info',title:'Sin insumos disponibles',text:'No se encontraron insumos disponibles para descontar del inventario.'});
    const options = insumos.map(i=>`<option value="${i.id}">${escapeHtml(i.code||i.barcode||i.item)} · ${escapeHtml(i.item||'Insumo')} · Stock: ${i.stock ?? '1'}</option>`).join('');
    const res=await Swal.fire({title:'Registrar insumo utilizado',html:`<label class="swal-label">Insumo</label><select id="consume_item" class="swal2-select">${options}</select><label class="swal-label">Cantidad</label><input id="consume_qty" type="number" min="1" value="1" class="swal2-input"><label class="swal-label">Observación</label><textarea id="consume_obs" class="swal2-textarea"></textarea>`,showCancelButton:true,confirmButtonText:'Descontar del inventario',preConfirm:()=>({id:document.getElementById('consume_item').value, qty:Number(document.getElementById('consume_qty').value||1), obs:document.getElementById('consume_obs').value})});
    if(!res.isConfirmed) return;
    const item = state.inventory.find(i=>String(i.id)===String(res.value.id));
    if(item){ item.stock = Math.max(0, Number(item.stock ?? 1) - res.value.qty); if(item.stock===0 && String(item.type||'').toLowerCase().includes('insumo')) item.status='Sin stock'; o.usedItems=o.usedItems||[]; o.usedItems.push({id:item.id, code:item.code, item:item.item, qty:res.value.qty, obs:res.value.obs, date:new Date().toLocaleString('es-AR')}); addHistory(o, `Uso de insumo: ${item.item || item.code} x${res.value.qty}. Stock actualizado.`); persistOrders(); try{localStorage.setItem('ism_inventory', JSON.stringify(state.inventory));}catch(e){} renderView(); }
  }

  async function writeOrderNote(o){
    const aiText = `A la Dirección/Área de Compras:\n\nSe solicita evaluar la provisión de insumos/repuestos necesarios para dar continuidad a la Orden de Servicio OS-${String(o.order).padStart(5,'0')}, correspondiente a la repartición ${o.client||'sin especificar'}.\n\nEquipo: ${[o.type,o.brand,o.model].filter(Boolean).join(' · ') || 'sin especificar'}\nN° de serie: ${o.serial||'sin especificar'}\nFalla declarada: ${o.failure||'sin especificar'}\nInforme técnico: ${o.report||'pendiente de completar'}\n\nLa solicitud se emite a los fines de garantizar la trazabilidad, continuidad operativa y resolución técnica del caso.\n\nAtentamente,\nÁrea de Soporte Técnico - Dirección de Informática`;
    const res=await Swal.fire({title:'Nota institucional',width:850,html:`<p class="muted">Redacción manual o asistida por IA local basada en la Orden de Servicio.</p><textarea id="order_note_text" class="swal2-textarea note-textarea">${escapeHtml(aiText)}</textarea>`,showCancelButton:true,confirmButtonText:'Imprimir nota',cancelButtonText:'Cerrar'});
    if(res.isConfirmed){ const text=document.getElementById('order_note_text').value; addHistory(o,'Generación de nota institucional para compra/insumos.'); const w=window.open('','_blank'); w.document.write(`<html><head><title>Nota OS-${o.order}</title><style>body{font-family:Arial;margin:40px;line-height:1.55;white-space:pre-wrap}</style></head><body>${escapeHtml(text)}<script>print()<\/script></body></html>`); w.document.close(); }
  }

  async function importServiceOrdersSmart(){
    const res = await Swal.fire({title:'Importador inteligente SATMANAGER',width:820,html:`<p class="muted">Seleccione un respaldo exportado desde Access/SATMANAGER. En GitHub Pages se importan CSV/JSON de forma directa. Para archivos .MDB/.ACCDB, use el conversor incluido en <strong>tools/access_to_csv.py</strong> o exporte la tabla Reparaciones a CSV desde Access.</p><input id="so_import_file" type="file" class="swal2-file" accept=".csv,.json,.mdb,.accdb"><div class="hint-box">Mapeo flexible: Orden, Fecha, Cliente, Dirección, Teléfono, Técnico, Número de Serie, Tipo de Equipo, Marca, Modelo, Accesorios, Falla, Informe y Estado.</div>`,showCancelButton:true,confirmButtonText:'Importar'});
    if(!res.isConfirmed) return;
    const file=document.getElementById('so_import_file').files[0]; if(!file) return;
    const ext=file.name.split('.').pop().toLowerCase();
    if(['mdb','accdb'].includes(ext)){ await Swal.fire({icon:'info',title:'Respaldo Access detectado',html:'El navegador no puede leer directamente bases Access por seguridad. Se agregó el conversor <b>tools/access_to_csv.py</b> y el instructivo en README. Exporte la tabla de reparaciones a CSV y vuelva a importarla aquí.'}); return; }
    const text=await file.text(); let records=[];
    if(ext==='json') records=JSON.parse(text); else {
      const lines=text.split(/\r?\n/).filter(Boolean); const headers=lines.shift().split(/[;,]/).map(h=>h.trim().toLowerCase());
      records=lines.map(line=>{const cells=line.split(/[;,]/); const o={}; headers.forEach((h,i)=>o[h]=cells[i]||''); return o;});
    }
    const pick=(r,names)=>{ for(const n of names){ const k=Object.keys(r).find(k=>k.toLowerCase().includes(n)); if(k) return r[k]; } return ''; };
    const imported=records.map((r,i)=>({id:Date.now()+i,order:pick(r,['orden','order','nro'])||nextOrderNumber()+i,date:(pick(r,['fecha','date'])||new Date().toISOString().slice(0,10)).split(' ')[0],time:pick(r,['hora','time'])||'',client:pick(r,['cliente','repart','oficina','solicit']),address:pick(r,['direccion','dirección']),phone:pick(r,['telefono','teléfono','celular']),technician:pick(r,['tecnico','técnico']),serial:pick(r,['serie','serial']),type:pick(r,['tipo']),brand:pick(r,['marca']),model:pick(r,['modelo']),accessories:pick(r,['accesorio']),failure:pick(r,['falla','problema']),report:pick(r,['informe','reparacion','reparación']),status:pick(r,['estado'])||'Pendiente',source_ticket:pick(r,['ticket']),usedItems:[],history:[{date:new Date().toLocaleString('es-AR'),user:currentUserLabel(),action:'Importado desde respaldo SATMANAGER/CSV'}]}));
    state.serviceOrders=[...imported,...state.serviceOrders]; persistOrders(); notify('Importación SATMANAGER',`Se importaron ${imported.length} órdenes de servicio desde ${file.name}.`); renderView();
  }

  async function createOrderFromTicket(){
    const pending=(state.supportTickets||[]).find(t=>!['Resuelto','Cerrado','Entregado'].includes(t.status));
    if(!pending) return Swal.fire({icon:'info',title:'Sin tickets pendientes',text:'No hay tickets disponibles para generar una orden técnica.'});
    await openServiceOrderForm({id:Date.now(),order:nextOrderNumber(),date:new Date().toISOString().slice(0,10),time:new Date().toTimeString().slice(0,5),client:pending.room||pending.office||pending.requester_name||'Oficina solicitante',address:'',phone:pending.requester_phone||'',technician:state.user?.name||'',serial:pending.serial||'',type:pending.asset_label||pending.equipment||'',brand:'',model:'',accessories:'',failure:pending.description||pending.incident_type||'',report:'',status:'Pendiente',source_ticket:pending.ticket_number||pending.id||'',usedItems:[],history:[]});
    if(pending) notify('Ticket en atención',`El ticket ${pending.ticket_number||pending.id||''} fue tomado para generar Orden de Servicio. Fecha/hora: ${new Date().toLocaleString('es-AR')}. Usuario: ${currentUserLabel()}.`);
  }

  function exportCurrentPdf(){
    const title=(officialLabels[state.currentView]?.[0] || 'Reporte'); const now=new Date().toLocaleString('es-AR'); const logo='https://www.educaciontuc.gov.ar/wp-content/uploads/2024/10/MINISTERIO-DE-EDUCACION-blanco.png';
    const content=document.querySelector('#appContent')?.innerHTML || '';
    const w=window.open('','_blank'); w.document.write(`<html><head><title>${title}</title><style>body{font-family:Arial;margin:24px;color:#111}.rotulo{display:flex;align-items:center;gap:16px;border-bottom:2px solid #111;padding-bottom:10px;margin-bottom:18px}.rotulo img{max-height:64px;background:#111;padding:5px}.muted,.toolbar,button,input,select{display:none!important}table{width:100%;border-collapse:collapse;font-size:11px}td,th{border:1px solid #aaa;padding:5px}.card{border:0}.status-pill{border:1px solid #aaa;padding:2px 6px;border-radius:10px}</style></head><body><div class="rotulo"><img src="${logo}"><div><h1>${title}</h1><p>Fecha y hora del reporte: ${now}</p></div></div>${content}<script>print()<\/script></body></html>`); w.document.close();
  }
  function enhancePdfButtons(){
    document.querySelectorAll('[data-export]').forEach(btn=>{ const key=btn.getAttribute('data-export'); const tb=btn.parentElement; if(tb && !tb.querySelector(`[data-pdf-export="${key}"]`)){ const b=document.createElement('button'); b.className='btn btn-secondary btn-sm'; b.setAttribute('data-pdf-export',key); b.innerHTML=(icons.pdf||'')+' PDF A4'; tb.appendChild(b); }});
  }

  const oldRenderSettings = renderSettings;
  renderSettings = function(){
    const base = oldRenderSettings();
    return base + `<section class="card glass palette-card"><div class="section-header"><div><h3>Paleta visual de la PWA</h3><p class="muted">Personalice los colores institucionales sin tocar código.</p></div><button class="btn btn-primary btn-sm" id="savePaletteBtn">Guardar paleta</button></div><div class="palette-grid"><label>Primario<input type="color" id="colorPrimary" value="${localStorage.getItem('tm_color_primary')||'#7c5cff'}"></label><label>Secundario<input type="color" id="colorAccent" value="${localStorage.getItem('tm_color_accent')||'#22d3ee'}"></label><label>Fondo<input type="color" id="colorBg" value="${localStorage.getItem('tm_color_bg')||'#081225'}"></label></div></section>`;
  };
  ...
  renderView = function(){
    ...
    appContent.innerHTML = (views[state.currentView] || renderDashboard)();
    attachViewEvents(); enhancePdfButtons(); renderNotificationDropdown(); renderCharts();
  };
  ...
  attachViewEvents = function(){
    ...
    document.getElementById('importServiceOrdersBtn')?.addEventListener('click',importServiceOrdersSmart);
    document.getElementById('smartAccessImportHelpBtn')?.addEventListener('click',()=>Swal.fire({title:'Importación desde Access/SATMANAGER',html:'Use <b>tools/access_to_csv.py</b> con Python + mdbtools o exporte desde Access a CSV. Luego importe el CSV en este módulo. El mapeo admite nombres de columnas similares.',icon:'info'}));
    document.getElementById('newServiceOrderNoteBtn')?.addEventListener('click',()=>{const o=state.serviceOrders[0]; if(o) writeOrderNote(o);});
    document.querySelectorAll('[data-pdf-export]').forEach(b=>b.addEventListener('click',exportCurrentPdf));
    document.querySelectorAll('[data-so-view]').forEach(b=>b.addEventListener('click',()=>{const o=state.serviceOrders.find(x=>String(x.id)===String(b.dataset.soView)); if(o) Swal.fire({title:'Detalle OS-'+String(o.order).padStart(5,'0'),width:760,html:`<div class="order-detail"><p><strong>Repartición:</strong> ${escapeHtml(o.client||'-')}</p><p><strong>Equipo:</strong> ${escapeHtml([o.type,o.brand,o.model].filter(Boolean).join(' · ')||'-')}</p><p><strong>Serie:</strong> ${escapeHtml(o.serial||'-')}</p><p><strong>Falla:</strong> ${escapeHtml(o.failure||'-')}</p><p><strong>Informe:</strong> ${escapeHtml(o.report||'-')}</p><p><strong>Estado:</strong> ${orderStatusPill(o.status)}</p></div>`});}));
    ...
    document.querySelectorAll('[data-so-status]').forEach(b=>b.addEventListener('click',()=>{const o=state.serviceOrders.find(x=>String(x.id)===String(b.dataset.soStatus)); if(o) changeOrderStatus(o);}));
    document.querySelectorAll('[data-so-print-in]').forEach(b=>b.addEventListener('click',()=>{const o=state.serviceOrders.find(x=>String(x.id)===String(b.dataset.soPrintIn)); if(o) printOrder(o,'ingreso');}));
    document.querySelectorAll('[data-so-print-out]').forEach(b=>b.addEventListener('click',()=>{const o=state.serviceOrders.find(x=>String(x.id)===String(b.dataset.soPrintOut)); if(o) printOrder(o,'entrega');}));
    document.querySelectorAll('[data-so-history]').forEach(b=>b.addEventListener('click',()=>{const o=state.serviceOrders.find(x=>String(x.id)===String(b.dataset.soHistory)); if(o) Swal.fire({title:'Historia técnica del equipo',html:`<div class="table-wrap"><table class="table"><thead><tr><th>Fecha/Hora</th><th>Usuario</th><th>Acción</th></tr></thead><tbody>${(o.history||[]).map(h=>`<tr><td>${escapeHtml(h.date)}</td><td>${escapeHtml(h.user)}</td><td>${escapeHtml(h.action)}</td></tr>`).join('')||'<tr><td colspan="3">Sin movimientos registrados.</td></tr>'}</tbody></table></div>`,width:850});}));
    document.querySelectorAll('[data-so-note]').forEach(b=>b.addEventListener('click',()=>{const o=state.serviceOrders.find(x=>String(x.id)===String(b.dataset.soNote)); if(o) writeOrderNote(o);}));
    document.querySelectorAll('[data-so-consume]').forEach(b=>b.addEventListener('click',()=>{const o=state.serviceOrders.find(x=>String(x.id)===String(b.dataset.soConsume)); if(o) consumeInventory(o);}));
    document.querySelectorAll('[data-so-delete]').forEach(b=>b.addEventListener('click',async()=>{const id=b.dataset.soDelete; const r=await Swal.fire({icon:'warning',title:'¿Eliminar orden?',text:'Se eliminará la trazabilidad de la orden seleccionada.',showCancelButton:true,confirmButtonText:'Eliminar'}); if(r.isConfirmed){state.serviceOrders=state.serviceOrders.filter(o=>String(o.id)!==String(id)); persistOrders(); notify('Orden eliminada',`Se eliminó una Orden de Servicio. Usuario: ${currentUserLabel()}.`); renderView();}}));
    document.getElementById('savePaletteBtn')?.addEventListener('click',()=>{ const p=document.getElementById('colorPrimary').value, a=document.getElementById('colorAccent').value, bg=document.getElementById('colorBg').value; localStorage.setItem('tm_color_primary',p); localStorage.setItem('tm_color_accent',a); localStorage.setItem('tm_color_bg',bg); document.documentElement.style.setProperty('--primary',p); document.documentElement.style.setProperty('--accent',a); document.documentElement.style.setProperty('--bg',bg); showToast('Paleta guardada','Se aplicaron los colores seleccionados.'); });
  };
  ...
  ['tm_color_primary','tm_color_accent','tm_color_bg'].forEach(k=>{const v=localStorage.getItem(k); if(v) document.documentElement.style.setProperty(k==='tm_color_primary'?'--primary':k==='tm_color_accent'?'--accent':'--bg',v);});
