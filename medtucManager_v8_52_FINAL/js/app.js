...
/* ========================= v8.50 HOTFIX FINAL: menú RBAC real, scope Técnico/Referente, notificaciones por usuario ========================= */
(function(){
  const $ = (s,r=document)=>r.querySelector(s);
  const $$ = (s,r=document)=>Array.from(r.querySelectorAll(s));
  const E = (v)=>String(v??'').replace(/[&<>'"]/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;',"'":'&#39;','"':'&quot;'}[m]));
  const fmt = (v)=>{ try{return v?new Date(v).toLocaleString('es-AR'):'-'}catch(_){return '-'} };
  const norm = (v)=>String(v||'').normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase().trim();
  const role = ()=>String(window.state?.profile?.role_name||'Usuarios').trim();
  const roleN = ()=>norm(role());
  const uid = ()=>String(window.state?.user?.id||window.state?.profile?.id||'');
  const email = ()=>String(window.state?.profile?.email||window.state?.user?.email||'').toLowerCase().trim();
  const fullName = ()=>norm(window.state?.profile?.full_name||'');
  const isSuper = ()=>['superadmin','super admin'].includes(roleN());
  const isAdmin = ()=>['superadmin','super admin','admin','superuser','super user'].includes(roleN());
  const isTech = ()=>['tecnicos','tecnico','técnicos','técnico','profesional tecnico','profesional técnico'].includes(roleN());
  const isRef = ()=>['referente','referentes'].includes(roleN());
  const MODULE_LABELS={dashboard:'Dashboard',users:'Usuarios',roles:'Roles y Permisos',orders:'Órdenes de Servicio',inventory:'Inventario',loans:'Gestión de Préstamos',tickets:'Soporte Ticket',notifications:'Notificaciones',offices:'Dtos/Oficinas',exceptions:'Excepciones',logs:'Registros Historial',profile:'Mi Perfil',settings:'Configuraciones',logout:'Cerrar Sesión'};
  const SYN={tickets:['tickets','support_tickets','soporte_ticket','soporte ticket','Soporte Ticket'],orders:['orders','service_orders','ordenes','órdenes','ordenes_servicio','Órdenes de Servicio'],notifications:['notifications','notificaciones'],users:['users','usuarios'],roles:['roles','permisos'],inventory:['inventory','inventario'],loans:['loans','prestamos','préstamos'],offices:['offices','dtos/oficinas'],exceptions:['exceptions','excepciones'],logs:['logs','registros']};
  function alias(m){ const n=norm(m); for(const [k,arr] of Object.entries(SYN)){ if(arr.map(norm).includes(n)) return k; } return n; }
  function getMods(){ return (window.modules || (typeof modules!=='undefined'?modules:[])).filter(Boolean); }
  function getPermMap(){
    const out={};
    const src=window.rolePermsV830||window.rolePermsV845||window.rolePermsV848||{};
    Object.entries(src||{}).forEach(([r,mods])=>{ out[r]=out[r]||{}; Object.entries(mods||{}).forEach(([m,acts])=>{ out[r][alias(m)]=Array.isArray(acts)?acts:Object.keys(acts||{}).filter(a=>acts[a]); }); });
    return out;
  }
  window.canModuleV850=function(module,action='view'){
    const m=alias(module), a=String(action||'view');
    if(['logout','profile','settings'].includes(m)) return true;
    if(m==='dashboard') return true;
    if(isSuper()) return true;
    const p=getPermMap();
    const names=[role(), role().endsWith('s')?role().slice(0,-1):role()+'s'];
    for(const rn of names){ const acts=p[rn]?.[m] || p[Object.keys(p).find(k=>norm(k)===norm(rn))]?.[m]; if(acts && (acts.includes(a)||acts.includes('view'))) return a==='view' || acts.includes(a); }
    // Fallback operativo para perfiles técnicos/referentes cuando la tabla de permisos aún no sincronizó en cliente.
    if(isTech()) return ['orders','tickets','notifications'].includes(m) && ['view','export'].includes(a);
    if(isRef()) return ['tickets','notifications'].includes(m) && ['view','create','export'].includes(a);
    return false;
  };
  window.canModuleV830=window.canModuleV834=window.canModuleV845=window.canModuleV847=window.canModuleV848=window.canModuleV850;
  function allowedModules850(){
    const base=getMods().filter(m=>m && m[0] !== 'logout');
    const allowed=base.filter(m=>window.canModuleV850(m[0],'view'));
    const ensure=(key,label,icon)=>{ if(!allowed.some(m=>m[0]===key)) allowed.push([key,label,icon||'']); };
    ensure('dashboard','Dashboard',(window.ico&&ico.dashboard)||'');
    ensure('profile','Mi Perfil',(window.ico&&ico.user)||'');
    ensure('settings','Configuraciones',(window.ico&&ico.settings)||'');
    allowed.push(['logout','Cerrar Sesión',(window.ico&&(ico.logout||ico.exit))||'']);
    const order=['dashboard','users','roles','orders','inventory','loans','tickets','notifications','offices','exceptions','logs','profile','settings','logout'];
    return allowed.filter((m,i,a)=>a.findIndex(x=>x[0]===m[0])===i).sort((a,b)=>order.indexOf(a[0])-order.indexOf(b[0]));
  }
  window.allowedModules850=allowedModules850; window.allowedModules848=allowedModules850;
  function injectCss850(){ if($('#v850css'))return; const st=document.createElement('style'); st.id='v850css'; st.textContent=`
    .notification-popover{position:fixed;z-index:99999;width:min(420px,calc(100vw - 24px));max-height:min(620px,calc(100vh - 110px));background:linear-gradient(180deg,#13213a,#0d1729);border:1px solid rgba(148,163,184,.28);border-radius:18px;box-shadow:0 22px 60px rgba(0,0,0,.45);padding:12px;color:#f8fafc;overflow:hidden}.notification-popover .np-head{display:flex;align-items:center;gap:8px;border-bottom:1px solid rgba(148,163,184,.18);padding-bottom:10px}.notification-popover .np-head b{flex:1}.notification-popover .np-head button{border:1px solid rgba(148,163,184,.25);background:rgba(255,255,255,.06);color:#fff;border-radius:10px;padding:8px 10px;font-weight:800}.notification-popover .np-close{font-size:20px;line-height:1;padding:4px 10px!important}.notification-popover .np-items{max-height:calc(min(620px,100vh - 110px) - 74px);overflow-y:auto;padding-right:4px}.notification-popover .np-item{padding:12px;border:1px solid rgba(148,163,184,.18);border-radius:12px;margin:10px 0;background:rgba(15,23,42,.55);cursor:pointer}.notification-popover .np-item b,.notification-popover .np-item small,.notification-popover .np-item em{display:block}.notification-popover .np-item small{color:#cbd5e1;margin-top:5px}.notification-popover .np-item em{color:#93c5fd;font-style:normal;font-size:12px;margin-top:6px}
    .calendar-grid .cal-cell span{cursor:pointer}.calendar-grid .cal-cell span:hover{filter:brightness(1.15);transform:translateY(-1px)}
    .locked-note{margin-top:8px;color:#93c5fd;font-size:12px}.profile-schedule-locked{opacity:.65}
    @media(max-width:760px){.notification-popover{left:10px!important;right:10px!important;top:82px!important;width:auto;max-height:70vh}.notification-popover .np-head{align-items:flex-start}.notification-popover .np-head button{font-size:12px;padding:7px 8px}.notification-popover .np-items{max-height:calc(70vh - 72px)}}
  `; document.head.appendChild(st); }
  function bindNotify850(){
    injectCss850(); const btn=$('#notifyBtn'); if(!btn || btn.dataset.v850)return; btn.dataset.v850='1';
    btn.addEventListener('mouseenter',()=>window.showNotificationPreviewV850()); btn.addEventListener('click',(e)=>{e.preventDefault(); window.showNotificationPreviewV850();});
  }
  ...
  window.renderShell=function(){
    ...
    const logo=(localStorage.theme==='light'?state.settings?.logo_light_url:state.settings?.logo_dark_url)||'assets/logo.svg';
    const avatar=state.profile?.avatar_url||'assets/avatar-default.svg';
    const allowed=allowedModules850(); const mobileMain=allowed.filter(m=>['dashboard','orders','tickets','notifications','profile'].includes(m[0])).slice(0,4);
    const themeIcon=localStorage.theme==='light'?(typeof themeMoonSvg==='function'?themeMoonSvg():'☾'):(typeof themeSunSvg==='function'?themeSunSvg():'☼');
    $('#app').innerHTML=`<div class="layout"><aside class="sidebar"><div class="brand" id="brandToggle" title="Colapsar/expandir menú"><img src="${E(logo)}" onerror="this.src='assets/logo.svg'"><div><strong>${E(state.settings?.institution_name||'Dirección de Informática')}</strong><small>${E(state.settings?.institution_area||'Área Soporte Técnico')}</small></div></div><nav class="nav">${allowed.map(m=>`<button data-page="${E(m[0])}">${m[2]||''}<span>${E(MODULE_LABELS[m[0]]||m[1])}</span></button>`).join('')}</nav><div class="sidebar-foot"><div>TICKET & OS MANAGER © 2026</div><a class="cafecito" href="https://cafecito.app/fmgambino" rel="noopener" target="_blank">Invítame un Cafecito</a></div></aside><main class="main"><section class="topbar"><div class="mobile-brand-top"><img src="${E(logo)}" onerror="this.src='assets/logo.svg'"></div><div class="topbar-title"><h1 id="pageTitle"></h1><p id="pageDesc"></p></div><div class="actions"><button class="icon-btn desktop-only theme-svg" id="themeBtn" title="Cambiar tema">${themeIcon}</button><button class="icon-btn desktop-only" id="fullBtn" title="Pantalla completa">⛶</button><button class="icon-btn notify-trigger" id="notifyBtn" title="Notificaciones">${ico?.bell||'🔔'}<span class="notif-badge-count" hidden>0</span></button><button class="avatar-action" id="avatarBtn" title="Mi perfil"><img src="${E(avatar)}" onerror="this.src='assets/avatar-default.svg'"><span class="desktop-only">${E(state.profile?.full_name||state.profile?.email||'')}</span></button><button class="icon-btn logout-action" id="logoutBtn" title="Cerrar sesión">${ico?.logout||ico?.exit||'⇲'}</button></div></section><section id="content"></section><footer class="footer">TICKET MANAGER © 2026 Tucumán - Argentina<br>by <a href="https://github.com/fmgambino" target="_blank" rel="noopener">Ing. Fernando Gambino</a> · Todos los Derechos Registrados</footer></main><nav class="mobile-dock" aria-label="Menú principal móvil">${mobileMain.map(m=>`<button data-page="${E(m[0])}" aria-label="${E(m[1])}">${m[2]||''}<span>${E(MODULE_LABELS[m[0]]||m[1])}</span></button>`).join('')}<button id="mobileMoreBtn" aria-label="Más módulos"><svg class="icon" viewBox="0 0 24 24"><path d="M4 6h16M4 12h16M4 18h16"/></svg><span>Más</span></button></nav></div>`;
    $$('.nav button,.mobile-dock button[data-page]').forEach(b=>b.onclick=()=>window.route(b.dataset.page));
    $('#themeBtn').onclick=()=>{localStorage.theme=localStorage.theme==='light'?'dark':'light'; if(typeof applySettings==='function')applySettings(); window.renderShell(); window.route(state.page||'dashboard');};
    $('#fullBtn').onclick=()=>document.fullscreenElement?document.exitFullscreen():document.documentElement.requestFullscreen?.();
    $('#avatarBtn').onclick=()=>window.route('profile'); $('#logoutBtn').onclick=()=>window.route('logout'); $('#brandToggle').onclick=()=>document.body.classList.toggle('sidebar-collapsed');
    $('#mobileMoreBtn').onclick=()=>{ const rest=allowed.filter(m=>!mobileMain.some(x=>x[0]===m[0])); Swal.fire({title:'Menú',html:`<div class="mobile-menu-sheet">${rest.map(m=>`<button class="mobile-menu-item" data-page="${E(m[0])}">${m[2]||''}<span>${E(MODULE_LABELS[m[0]]||m[1])}</span></button>`).join('')}</div>`,showConfirmButton:false,showCloseButton:true,customClass:{popup:'tm-modal mobile-menu-modal'}}); setTimeout(()=>$$('.mobile-menu-item').forEach(b=>b.onclick=()=>{Swal.close();window.route(b.dataset.page);}),50); };
    bindNotify850(); window.updateNotificationCountV850?.(); if(typeof syncNavActive==='function')syncNavActive(); else $$('.nav button,.mobile-dock button[data-page]').forEach(b=>b.classList.toggle('active',b.dataset.page===state.page));
  }; try{renderShell=window.renderShell}catch(_){ }
  function assignedToMeOrder(o){
    const me=uid(), nm=fullName(), em=email();
    const ids=[o.assigned_to,o.technician_user_id,o.collaborator_assigned_to,o.attended_by,o.created_by,o.updated_by].filter(Boolean).map(String);
    if(ids.includes(me)) return true;
    const names=[o.professional_technician,o.technician_name,o.attended_by_name].map(norm);
    if(nm && names.some(x=>x===nm || x.includes(nm) || nm.includes(x))) return true;
    const emails=[o.technician_email,o.assigned_email].map(x=>String(x||'').toLowerCase());
    return em && emails.includes(em);
  }
  function visibleOrders(rows){ if(isSuper()||isAdmin()) return rows||[]; if(isTech()) return (rows||[]).filter(assignedToMeOrder); if(isRef()) return (rows||[]).filter(o=>String(o.requester_email||'').toLowerCase()===email() || String(o.created_by||'')===uid()); return []; }
  function visibleTickets(rows,orders){
    if(isSuper()||isAdmin()) return rows||[]; const orderIds=new Set((orders||[]).map(o=>String(o.id)));
    if(isTech()) return (rows||[]).filter(t=>[t.assigned_to,t.technician_user_id,t.collaborator_assigned_to,t.attended_by].filter(Boolean).map(String).includes(uid()) || orderIds.has(String(t.service_order_id)));
    if(isRef()) return (rows||[]).filter(t=>String(t.created_by||'')===uid() || String(t.requester_email||'').toLowerCase()===email() || norm(t.requester_name)===fullName());
    return [];
  }
  function statusName(o){return o.service_order_statuses?.name||o.status||'Sin estado'}
  function finalOrder(o){return ['terminada','entregada','cerrada','finalizada','lista p/retirar'].includes(norm(statusName(o)))}
  function delayedOrder(o){const t=new Date(o.received_at||o.created_at||Date.now()).getTime(); return Date.now()-t>7*86400000 && !finalOrder(o)}
  async function allRows(){
    const [or,tk,iv,us]=await Promise.all([
      supa.from('service_orders').select('*,service_order_statuses(name,color)').order('received_at',{ascending:false}).limit(5000),
      supa.from('support_tickets').select('*').order('created_at',{ascending:false}).limit(5000),
      supa.from('inventory_items').select('*').order('created_at',{ascending:false}).limit(5000),
      supa.from('profiles').select('*').order('full_name',{ascending:true}).limit(5000)
    ]);
    const orders=visibleOrders(or.data||[]); const tickets=visibleTickets(tk.data||[],orders); return {orders,tickets,inventory:iv.data||[],users:us.data||[]};
  }
  function cardsHtml(cards){return `<div class="dash-grid cards">${cards.map(c=>`<div class="stat-card" onclick="showDashCardV850('${c.key}')"><small>${E(c.title)}</small><strong>${E(c.value)}</strong><span>${E(c.sub)}</span></div>`).join('')}</div>`}
  function calendarHtml850(orders){
    const now=new Date(), cal=state.dashboardCalendar||{month:now.getMonth(),year:now.getFullYear()}, d=new Date(cal.year,cal.month,1), first=d.getDay()||7, days=new Date(cal.year,cal.month+1,0).getDate(); let cells='';
    for(let i=1;i<first;i++)cells+='<div class="cal-cell muted"></div>';
    for(let day=1;day<=days;day++){const os=(orders||[]).filter(o=>{const x=new Date(o.received_at||o.created_at);return x.getFullYear()===cal.year&&x.getMonth()===cal.month&&x.getDate()===day}); cells+=`<div class="cal-cell"><b>${day}</b>${os.slice(0,4).map(o=>`<span onclick="event.stopPropagation();viewOrderV850('${E(o.id)}')">OS ${E(o.order_number||o.satmanager_order||'')} · ${E(o.office||'')}</span>`).join('')}</div>`;}
    return `<div class="card calendar-card"><div class="module-head"><div><h2>Calendario de Órdenes de Servicio</h2><p>${d.toLocaleString('es-AR',{month:'long',year:'numeric'})}</p></div><div class="module-actions"><button class="btn" onclick="changeDashboardMonth(-1)">←</button><button class="btn" onclick="changeDashboardMonth(1)">→</button><label>Año <select onchange="changeDashboardYear(this.value)">${[cal.year-1,cal.year,cal.year+1].map(y=>`<option ${y===cal.year?'selected':''}>${y}</option>`).join('')}</select></label><button class="btn" onclick="route('orders')">Ver órdenes</button></div></div><div class="calendar-head"><span>LUN</span><span>MAR</span><span>MIÉ</span><span>JUE</span><span>VIE</span><span>SÁB</span><span>DOM</span></div><div class="calendar-grid">${cells}</div></div>`;
  }
  window.changeDashboardMonth=function(delta){const n=new Date(), c=state.dashboardCalendar||{month:n.getMonth(),year:n.getFullYear()}, d=new Date(c.year,c.month+Number(delta||0),1); state.dashboardCalendar={month:d.getMonth(),year:d.getFullYear()}; window.renderDashboardV850();};
  window.changeDashboardYear=function(y){const n=new Date(); state.dashboardCalendar=state.dashboardCalendar||{month:n.getMonth(),year:n.getFullYear()}; state.dashboardCalendar.year=Number(y)||n.getFullYear(); window.renderDashboardV850();};
  window.renderDashboardV850=async function(){
    injectCss850(); if(typeof page==='function') page('Dashboard','Vista ejecutiva del sistema institucional con indicadores, alertas y calendario de órdenes.');
    const rows=await allRows(); state.rows={...(state.rows||{}),...rows}; const {orders,tickets,inventory,users}=rows; const urgent=orders.filter(o=>norm(o.priority)==='urgente'), delayed=orders.filter(delayedOrder), pending=orders.filter(o=>!finalOrder(o)), done=orders.filter(finalOrder);
    const cards=(isTech()?[
      {key:'orders',title:'Órdenes',value:orders.length,sub:'Asignadas a mi usuario.'},{key:'urgent',title:'Urgentes',value:urgent.length,sub:'Prioridad crítica'},{key:'delayed',title:'Atrasadas',value:delayed.length,sub:'Más de 7 días abiertas'},{key:'tickets',title:'Tickets',value:tickets.length,sub:'Asignados a mi usuario.'}
    ]:isRef()?[
      {key:'tickets',title:'Mis Tickets',value:tickets.length,sub:'Solicitudes creadas por mi usuario.'},{key:'orders',title:'Mis Órdenes',value:orders.length,sub:'Órdenes vinculadas a mis tickets.'},{key:'pending',title:'Pendientes',value:pending.length,sub:'Seguimiento pendiente.'},{key:'done',title:'Finalizadas',value:done.length,sub:'Cerradas o entregadas.'}
    ]:[
      {key:'users',title:'Usuarios',value:users.length,sub:'Perfiles autorizados.'},{key:'orders',title:'Órdenes',value:orders.length,sub:'Órdenes técnicas registradas.'},{key:'urgent',title:'Urgentes',value:urgent.length,sub:'Prioridad crítica'},{key:'delayed',title:'Atrasadas',value:delayed.length,sub:'Más de 7 días abiertas'},{key:'tickets',title:'Tickets',value:tickets.length,sub:'Solicitudes recibidas'},{key:'inventory',title:'Inventario',value:inventory.length,sub:'Activos e insumos'}]);
    const statusMap={}; orders.forEach(o=>{const s=statusName(o);statusMap[s]=(statusMap[s]||0)+1});
    $('#content').innerHTML=`${cardsHtml(cards)}<div class="dashboard-main"><div class="card alert-card"><h2>Alertas operativas</h2><p>${isTech()?'Resumen de mis órdenes y tickets asignados.':isRef()?'Resumen de mis tickets y órdenes vinculadas.':'Resumen de órdenes urgentes, pendientes, atrasadas y listas para retirar.'}</p><div class="alert-grid"><b>${urgent.length} urgentes</b><b>${pending.length} pendientes</b><b>${delayed.length} atrasadas</b><b>${done.length} listas/terminadas</b></div><h3>Estados</h3>${Object.keys(statusMap).length?Object.entries(statusMap).map(([k,v])=>`<div class="status-row"><span>${E(k)}</span><b>${v}</b></div>`).join(''):'<p>Sin órdenes asignadas.</p>'}</div>${calendarHtml850(orders)}</div>`;
  }; window.renderDashboard=window.renderDashboardV850; try{renderDashboard=window.renderDashboardV850}catch(_){ }
  function actionBtns(key,id){return `<div class="dash-actions"><button class="icon-btn" title="Ver" onclick="${key==='orders'?`viewOrderV850('${id}')`:`viewRecordV850('${key}','${id}')`}">${ico?.view||'👁'}</button>${window.canModuleV850(key,'edit')?`<button class="icon-btn" title="Editar" onclick="Swal.close();route('${key}');setTimeout(()=>openForm('${key}','${id}'),350)">${ico?.edit||'✎'}</button>`:''}</div>`}
  window.showDashCardV850=function(kind){
    const orders=state.rows?.orders||[], tickets=state.rows?.tickets||[], inv=state.rows?.inventory||[], users=state.rows?.users||[]; let title='Detalle', heads=[], rows=[];
    if(kind==='users'){title='Usuarios contabilizados';heads=['Nombre','Email','Rol','Oficina','Acciones'];rows=users.map(u=>[u.full_name,u.email,u.role_name,u.office,actionBtns('users',u.id)]);}
    else if(['orders','urgent','delayed','pending','done'].includes(kind)){title=kind==='urgent'?'Órdenes urgentes':kind==='delayed'?'Órdenes atrasadas':kind==='pending'?'Órdenes pendientes':kind==='done'?'Órdenes finalizadas':'Órdenes contabilizadas'; let arr=orders; if(kind==='urgent')arr=orders.filter(o=>norm(o.priority)==='urgente'); if(kind==='delayed')arr=orders.filter(delayedOrder); if(kind==='pending')arr=orders.filter(o=>!finalOrder(o)); if(kind==='done')arr=orders.filter(finalOrder); heads=['Orden','Oficina','Profesional Técnico','Falla / Diagnóstico','Acciones'];rows=arr.slice(0,500).map(o=>['#'+(o.order_number||o.satmanager_order||''),o.office||'-',o.professional_technician||o.technician_name||'-',o.fault_description||'-',actionBtns('orders',o.id)]);}
    else if(kind==='tickets'){title='Tickets contabilizados';heads=['Ticket','Solicitante','Oficina','Estado','Acciones'];rows=tickets.slice(0,500).map(t=>[t.ticket_number||'-',t.requester_name||'-',t.office||'-',t.status||'-',actionBtns('tickets',t.id)]);}
    else {title='Inventario contabilizado';heads=['Código','Nombre','Categoría','Stock','Acciones'];rows=inv.slice(0,500).map(i=>[i.code||i.barcode||'-',i.name||'-',i.category||i.item_type||'-',i.stock??'-',actionBtns('inventory',i.id)]);}
    Swal.fire({title,html:`<div class="dash-popup-table"><table><thead><tr>${heads.map(h=>`<th>${E(h)}</th>`).join('')}</tr></thead><tbody>${rows.map(r=>`<tr>${r.map((c,i)=>`<td class="${i===r.length-1?'actions-cell':''}">${i===r.length-1?c:E(c)}</td>`).join('')}</tr>`).join('')||`<tr><td colspan="${heads.length}">Sin registros.</td></tr>`}</tbody></table></div><small>Mostrando hasta 500 registros.</small>`,width:1050,confirmButtonText:'OK',customClass:{popup:'tm-modal dash-popup'}});
  }; window.showDashCard=window.showDashboardPopup=window.showDashCardV849=window.showDashCardV850;
  window.viewOrderV850=async function(id){
    let o=(state.rows?.orders||[]).find(x=>String(x.id)===String(id)); if(!o){const r=await supa.from('service_orders').select('*,service_order_statuses(name,color)').eq('id',id).maybeSingle(); o=r.data;} if(!o)return;
    const h=await supa.from('service_order_history').select('*').eq('service_order_id',id).order('created_at',{ascending:false}).limit(20);
    Swal.fire({title:'Detalle de Orden de Servicio',html:`<div class="detail-card order-detail"><div class="order-num">Orden #${E(o.satmanager_order||o.order_number||'-')}</div><div class="detail-grid"><p><b>Ingreso</b><span>${E(fmt(o.received_at))}</span></p><p><b>Estado</b><span>${E(statusName(o))}</span></p><p><b>Profesional técnico</b><span>${E(o.professional_technician||o.technician_name||'-')}</span></p><p><b>Oficina/Repartición</b><span>${E(o.office||'-')}</span></p><p><b>Solicitante / Referente</b><span>${E(o.requester_name||'-')}</span></p><p><b>Dirección</b><span>${E(o.address||'-')}</span></p><p><b>Equipo</b><span>${E([o.equipment_type,o.brand,o.model].filter(Boolean).join(' · ')||'-')}</span></p><p><b>N° serie</b><span>${E(o.serial_number||'-')}</span></p><p class="full"><b>Accesorios / insumos</b><span>${E(o.accessories||'-')}</span></p><p class="full"><b>Falla / diagnóstico</b><span>${E(o.fault_description||'-')}</span></p><p class="full"><b>Informe técnico</b><span>${E(o.technical_report||'-')}</span></p><p class="full"><b>Solución / observaciones</b><span>${E(o.solution||o.observations||'-')}</span></p></div><h4>Historial</h4><div class="history-list">${(h.data||[]).map(x=>`<div><b>${E(fmt(x.created_at))}</b> · ${E(x.action||'')} ${E(x.previous_value||'')} ${x.new_value?'→ '+E(x.new_value):''}</div>`).join('')||'<small>Sin movimientos registrados.</small>'}</div></div>`,width:980,customClass:{popup:'tm-modal'}});
  }; window.viewOrderV849=window.viewOrderV850;
  const oldViewRecord=window.viewRecord;
  window.viewRecordV850=window.viewRecord=async function(key,id){ if(key==='orders')return window.viewOrderV850(id); return oldViewRecord?oldViewRecord(key,id):undefined; };
  window.showNotificationPreviewV850=async function(){
    injectCss850(); $('.notification-popover')?.remove(); let q=supa.from('notifications').select('*').eq('is_read',false).order('created_at',{ascending:false}).limit(50); if(!isSuper()) q=q.or(`target_user.eq.${uid()},target_role.eq.${role()},target_role.eq.${role().endsWith('s')?role().slice(0,-1):role()+'s'},target_role.is.null`); const {data=[]}=await q;
    const btn=$('#notifyBtn'), r=btn?.getBoundingClientRect?.()||{right:window.innerWidth-20,bottom:70}; const box=document.createElement('div'); box.className='notification-popover'; box.style.right=Math.max(10,window.innerWidth-r.right)+'px'; box.style.top=(r.bottom+8)+'px';
    box.innerHTML=`<div class="np-head"><b>Notificaciones sin leer</b><button onclick="document.querySelector('.notification-popover')?.remove();route('notifications')">Historial de Notificaciones</button><button class="np-close" title="Cerrar" onclick="document.querySelector('.notification-popover')?.remove()">×</button></div><div class="np-items">${(data||[]).map(n=>`<div class="np-item" onclick="document.querySelector('.notification-popover')?.remove();route('${E(n.module||'notifications')}')"><b>${E(n.title)}</b><small>${E(n.body||'')}</small><em>${E(fmt(n.created_at))}</em></div>`).join('')||'<p>Sin notificaciones sin leer.</p>'}</div>`;
    box.addEventListener('mouseleave',()=>setTimeout(()=>box.remove(),350)); document.body.appendChild(box);
  }; window.showNotificationPreview=window.showNotificationPreviewV850;
  window.updateNotificationCountV850=async function(){try{let q=supa.from('notifications').select('id',{count:'exact',head:true}).eq('is_read',false); if(!isSuper()) q=q.or(`target_user.eq.${uid()},target_role.eq.${role()},target_role.eq.${role().endsWith('s')?role().slice(0,-1):role()+'s'},target_role.is.null`); const {count}=await q; const b=$('#notifyBtn .notif-badge-count')||$('#notifyBtn .badge')||$('#notifyBtn span'); if(b){b.textContent=count||0;b.hidden=!count;}}catch(e){}};
  async function renderOrdersV850(){
    if(typeof page==='function')page('Órdenes de Servicio','Trazabilidad técnica institucional, sin importes ni conceptos comerciales.'); const r=await supa.from('service_orders').select('*,service_order_statuses(name,color)').order('received_at',{ascending:false}).limit(5000); const rows=visibleOrders(r.data||[]); state.rows={...(state.rows||{}),orders:rows};
    $('#content').innerHTML=`<div class="card"><div class="module-head"><div><h2>Órdenes técnicas</h2><p>${isTech()?'Solo órdenes asignadas a mi usuario técnico.':'Registro de ingreso, profesional técnico, oficina, estados, informe técnico, insumos utilizados, notas y entrega.'}</p></div><div class="module-actions">${isAdmin()?'<button class="btn primary" onclick="openForm(\'orders\')">Nueva orden</button>':''}<button class="btn" onclick="exportCsv('orders')">Exportar CSV</button><button class="btn" onclick="exportPdf('orders')">PDF A4</button></div></div><input class="search" placeholder="Buscar en órdenes..." oninput="filterRows(this.value)"><div class="table-wrap"><table><thead><tr><th>Orden</th><th>Ingreso</th><th>Oficina/Repartición</th><th>Profesional técnico</th><th>Equipo</th><th>Falla / Diagnóstico</th><th>Estado</th><th>Acciones</th></tr></thead><tbody id="rowsBody">${rows.map(o=>`<tr data-search="${E(Object.values(o).join(' ').toLowerCase())}"><td><b>#${E(o.order_number||o.satmanager_order||'-')}</b></td><td>${E(fmt(o.received_at))}</td><td>${E(o.office||'-')}</td><td>${E(o.professional_technician||o.technician_name||'-')}</td><td>${E([o.equipment_type,o.brand,o.model,o.serial_number].filter(Boolean).join(' · ')||'-')}</td><td>${E(o.fault_description||'-')}</td><td><span class="badge">${E(statusName(o))}</span></td><td class="row-actions">${actionBtns('orders',o.id)}</td></tr>`).join('')||'<tr><td colspan="8">Sin órdenes asignadas.</td></tr>'}</tbody></table></div></div>`;
  }
  async function renderNotificationsV850(){
    if(typeof page==='function')page('Notificaciones','Avisos automáticos, cambios de estado y auditoría operativa.'); let q=supa.from('notifications').select('*').order('created_at',{ascending:false}).limit(5000); if(!isSuper()) q=q.or(`target_user.eq.${uid()},target_role.eq.${role()},target_role.eq.${role().endsWith('s')?role().slice(0,-1):role()+'s'},target_role.is.null`); const {data=[]}=await q; state.rows={...(state.rows||{}),notifications:data};
    $('#content').innerHTML=`<div class="card"><div class="module-head"><div><h2>Centro de notificaciones</h2><p>${isSuper()?'Vista global.':'Solo notificaciones dirigidas a mi usuario o a mi rol.'}</p></div><div class="module-actions">${isSuper()?'<button class="btn primary" onclick="sendInternalNotification()">Enviar notificación</button>':''}<button class="btn" onclick="exportCsv('notifications')">Exportar CSV</button><button class="btn" onclick="exportPdf('notifications')">PDF A4</button></div></div><div class="notif-list">${data.map(n=>`<div class="notif-card ${n.is_read?'read':'unread'}"><b>${E(n.title)}</b><p>${E(n.body||'')}</p><small>${E(fmt(n.created_at))} · ${E(n.module||'')}</small><span class="badge">${n.is_read?'Leída':'Sin leer'}</span></div>`).join('')||'<p>Sin notificaciones.</p>'}</div></div>`;
  };
  window.renderNotificationsV850=window.renderNotifications=renderNotificationsV850;
  const prevRoute=window.route;
  window.route=async function(p){
    const k=String(p||'dashboard'); state.page=k; if(k==='logout') return prevRoute?prevRoute(p):undefined;
    if(!window.canModuleV850(k,'view')){ if(typeof page==='function')page(MODULE_LABELS[k]||k,''); $('#content').innerHTML='<div class="card"><h2>Acceso restringido</h2><p>Tu perfil no tiene permisos para ver este módulo.</p></div>'; return; }
    if(k==='dashboard') return window.renderDashboardV850(); if(k==='orders') return renderOrdersV850(); if(k==='notifications') return renderNotificationsV850(); return prevRoute?prevRoute(p):undefined;
  }; try{route=window.route}catch(_){ }
  document.addEventListener('click',()=>setTimeout(()=>{ if(!isSuper() && /Editar Usuario/i.test($('.swal2-title')?.textContent||'')){ $$('.u_day,#u_work_start,#u_work_end').forEach(el=>{el.disabled=true; el.closest('label')?.classList.add('profile-schedule-locked');}); const grid=$('.swal-grid'); if(grid && !$('.locked-note',grid)){const n=document.createElement('div');n.className='locked-note full';n.textContent='El horario laboral solo puede ser modificado por SuperAdmin.';grid.appendChild(n);} } },150),true);
  document.addEventListener('DOMContentLoaded',()=>setTimeout(()=>{injectCss850(); bindNotify850(); window.updateNotificationCountV850?.(); if(state?.page==='dashboard')window.renderDashboardV850();},500));
  setTimeout(()=>{injectCss850(); bindNotify850(); window.updateNotificationCountV850?.(); if(state?.page==='dashboard')window.renderDashboardV850();},1000);
})();

/* ========================= v8.51 FINAL: alta segura de usuarios borrados, scope Usuario/Referente y asignación técnica por tarea ========================= */
(function(){
  const $=(s,r=document)=>r.querySelector(s);
  const $$=(s,r=document)=>Array.from(r.querySelectorAll(s));
  const E=(v)=>String(v??'').replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[m]));
  const norm=(v)=>String(v||'').trim().toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'');
  const uid=()=>String(state?.user?.id||state?.profile?.id||'');
  const email=()=>String(state?.profile?.email||state?.user?.email||'').toLowerCase();
  const fullname=()=>norm(state?.profile?.full_name||'');
  const role=()=>String(state?.profile?.role_name||'Usuarios');
  const rnorm=()=>norm(role());
  const isSuper=()=>['superadmin','super admin'].includes(rnorm());
  const isAdmin=()=>['superadmin','super admin','admin','superuser','super user'].includes(rnorm());
  const isTech=()=>['tecnicos','tecnico','técnicos','técnico','profesional tecnico','profesional técnico'].includes(rnorm());
  const isRef=()=>['referente','referentes'].includes(rnorm());
  const isUser=()=>!isAdmin()&&!isTech()&&!isRef();
  const cfgObj=()=>window.cfg || (typeof cfg!=='undefined'?cfg:{});
  const client=()=>window.supa || (typeof supa!=='undefined'?supa:null);
  const fmt2=(v)=>{try{if(!v)return '-';const d=new Date(v);return isNaN(d)?E(v):d.toLocaleString('es-AR')}catch(_){return E(v)}};
  function addCss(){ if($('#v851css'))return; const st=document.createElement('style'); st.id='v851css'; st.textContent=`
    body:not(.dark) .tm-modal .swal2-popup, body:not(.dark) .swal2-popup.tm-modal{background:#ffffff!important;color:#0f172a!important;border:1px solid #dbe4f0!important}
    body:not(.dark) .tm-modal input, body:not(.dark) .tm-modal select, body:not(.dark) .tm-modal textarea{background:#ffffff!important;color:#0f172a!important;border:1px solid #cbd5e1!important}
    body:not(.dark) .tm-modal input::placeholder, body:not(.dark) .tm-modal textarea::placeholder{color:#64748b!important}
    body:not(.dark) .notification-popover{background:#ffffff!important;color:#0f172a!important;border-color:#dbe4f0!important;box-shadow:0 20px 55px rgba(15,23,42,.18)!important}
    body:not(.dark) .notification-popover .np-head{border-bottom-color:#e2e8f0!important} body:not(.dark) .notification-popover .np-head button{background:#f8fafc!important;color:#0f172a!important;border-color:#cbd5e1!important}
    body:not(.dark) .notification-popover .np-item{background:#f8fafc!important;border-color:#dbe4f0!important;color:#0f172a!important} body:not(.dark) .notification-popover .np-item small{color:#475569!important}
    .alert-grid b{display:block;padding:14px 14px;border-radius:14px;background:rgba(148,163,184,.09);border:1px solid rgba(148,163,184,.22);border-left-width:7px}.alert-grid .op-urgente{border-left-color:#ef4444}.alert-grid .op-pendiente{border-left-color:#f59e0b}.alert-grid .op-atrasada{border-left-color:#dc2626}.alert-grid .op-lista{border-left-color:#22c55e}
    .task-scope-box{border:1px solid rgba(124,92,255,.25);border-radius:14px;padding:12px;margin-top:8px;background:rgba(124,92,255,.07)}.task-scope-box h4{margin:0 0 8px}.task-scope-box .checkline{display:inline-flex;margin:4px 10px 4px 0}
  `; document.head.appendChild(st); }
  window.addEventListener('DOMContentLoaded',addCss); setTimeout(addCss,300);

  function assignedOrder(o){ const ids=[o.assigned_to,o.technician_user_id,o.collaborator_assigned_to,o.attended_by].filter(Boolean).map(String); if(ids.includes(uid()))return true; const names=[o.professional_technician,o.technician_name,o.attended_by_name].map(norm); return fullname() && names.some(n=>n && (n===fullname()||n.includes(fullname())||fullname().includes(n))); }
  function ownOrder(o){ return String(o.requester_email||'').toLowerCase()===email() || String(o.created_by||'')===uid() || norm(o.requester_name)===fullname(); }
  function visibleOrders851(rows){ if(isAdmin())return rows||[]; if(isTech())return (rows||[]).filter(assignedOrder); return (rows||[]).filter(ownOrder); }
  function visibleTickets851(rows,orders){ if(isAdmin())return rows||[]; const orderIds=new Set((orders||[]).map(o=>String(o.id))); if(isTech())return (rows||[]).filter(t=>[t.assigned_to,t.technician_user_id,t.collaborator_assigned_to,t.attended_by].filter(Boolean).map(String).includes(uid()) || orderIds.has(String(t.service_order_id||''))); return (rows||[]).filter(t=>String(t.created_by||'')===uid() || String(t.requester_email||'').toLowerCase()===email() || norm(t.requester_name)===fullname() || orderIds.has(String(t.service_order_id||''))); }
  const finalOrder=(o)=>['terminada','entregada','cerrada','finalizada','lista p/retirar','lista','listo'].includes(norm(o.service_order_statuses?.name||o.status||''));
  const delayedOrder=(o)=>Date.now()-new Date(o.received_at||o.created_at||Date.now()).getTime()>7*86400000 && !finalOrder(o);
  async function allRows851(){ const s=client(); const [or,tk,iv,us]=await Promise.all([s.from('service_orders').select('*,service_order_statuses(name,color)').order('received_at',{ascending:false}).limit(5000),s.from('support_tickets').select('*').order('created_at',{ascending:false}).limit(5000),s.from('inventory_items').select('*').order('created_at',{ascending:false}).limit(5000),s.from('profiles').select('*').order('full_name',{ascending:true}).limit(5000)]); const orders=visibleOrders851(or.data||[]), tickets=visibleTickets851(tk.data||[],orders); return {orders,tickets,inventory:iv.data||[],users:us.data||[]}; }
  function calendarHtml(orders){ const now=new Date(), cal=state.dashboardCalendar||{month:now.getMonth(),year:now.getFullYear()}, d=new Date(cal.year,cal.month,1), first=d.getDay()||7, days=new Date(cal.year,cal.month+1,0).getDate(); let cells=''; for(let i=1;i<first;i++)cells+='<div class="cal-cell muted"></div>'; for(let day=1;day<=days;day++){const os=(orders||[]).filter(o=>{const x=new Date(o.received_at||o.created_at);return x.getFullYear()===cal.year&&x.getMonth()===cal.month&&x.getDate()===day}); cells+=`<div class="cal-cell"><b>${day}</b>${os.slice(0,4).map(o=>`<span onclick="event.stopPropagation();viewOrderV850&&viewOrderV850('${E(o.id)}')">OS ${E(o.order_number||o.satmanager_order||'')} · ${E(o.office||o.requester_name||'')}</span>`).join('')}</div>`;} return `<div class="card calendar-card"><div class="module-head"><div><h2>Calendario de Órdenes de Servicio</h2><p>${d.toLocaleString('es-AR',{month:'long',year:'numeric'})}</p></div><div class="module-actions"><button class="btn" onclick="changeDashboardMonth(-1)">←</button><button class="btn" onclick="changeDashboardMonth(1)">→</button><label>Año <select onchange="changeDashboardYear(this.value)">${[cal.year-1,cal.year,cal.year+1].map(y=>`<option ${y===cal.year?'selected':''}>${y}</option>`).join('')}</select></label><button class="btn" onclick="route('orders')">Ver órdenes</button></div></div><div class="calendar-head"><span>LUN</span><span>MAR</span><span>MIÉ</span><span>JUE</span><span>VIE</span><span>SÁB</span><span>DOM</span></div><div class="calendar-grid">${cells}</div></div>`; }
  window.renderDashboardV851=async function(){ addCss(); if(typeof page==='function')page('Dashboard','Vista ejecutiva del sistema institucional con indicadores, alertas y calendario de órdenes.'); const rows=await allRows851(); state.rows={...(state.rows||{}),...rows}; const {orders,tickets,inventory,users}=rows; const urgent=orders.filter(o=>norm(o.priority)==='urgente'), pending=orders.filter(o=>!finalOrder(o)), delayed=orders.filter(delayedOrder), done=orders.filter(finalOrder); const cards=isAdmin()? [{key:'orders',title:'Órdenes',value:orders.length,sub:'Órdenes técnicas registradas.'},{key:'urgent',title:'Urgentes',value:urgent.length,sub:'Prioridad crítica.'},{key:'delayed',title:'Atrasadas',value:delayed.length,sub:'Más de 7 días abiertas.'},{key:'tickets',title:'Tickets',value:tickets.length,sub:'Solicitudes recibidas.'},{key:'inventory',title:'Inventario',value:inventory.length,sub:'Activos e insumos.'}] : [{key:'tickets',title:isTech()?'Tickets asignados':'Mis Tickets',value:tickets.length,sub:isTech()?'Asignados a mi usuario.':'Cargados desde mi cuenta.'},{key:'orders',title:isTech()?'Órdenes asignadas':'Mis Órdenes',value:orders.length,sub:isTech()?'Asignadas a mi usuario.':'Vinculadas a mis tickets.'},{key:'pending',title:'Pendientes',value:pending.length,sub:'Seguimiento pendiente.'},{key:'done',title:'Listas/Terminadas',value:done.length,sub:'Finalizadas o listas.'}]; const statusMap={}; orders.forEach(o=>{const s=o.service_order_statuses?.name||o.status||'Sin estado';statusMap[s]=(statusMap[s]||0)+1}); $('#content').innerHTML=`<div class="dash-grid cards">${cards.map(c=>`<div class="stat-card" onclick="showDashCardV850&&showDashCardV850('${c.key}')"><small>${E(c.title)}</small><strong>${E(c.value)}</strong><span>${E(c.sub)}</span></div>`).join('')}</div><div class="dashboard-main"><div class="card alert-card"><h2>Alertas operativas</h2><p>${isAdmin()?'Resumen operativo global.':isTech()?'Resumen de mis órdenes y tickets asignados.':'Resumen de mis tickets y órdenes vinculadas.'}</p><div class="alert-grid"><b class="op-urgente">${urgent.length} urgentes</b><b class="op-pendiente">${pending.length} pendientes</b><b class="op-atrasada">${delayed.length} atrasadas</b><b class="op-lista">${done.length} listas/terminadas</b></div><h3>Estados</h3>${Object.keys(statusMap).length?Object.entries(statusMap).map(([k,v])=>`<div class="status-row"><span>${E(k)}</span><b>${v}</b></div>`).join(''):'<p>Sin órdenes para esta cuenta.</p>'}</div>${calendarHtml(orders)}</div>`; };
  window.renderDashboard=window.renderDashboardV851; try{renderDashboard=window.renderDashboardV851}catch(_){ }
  window.changeDashboardMonth=function(delta){const n=new Date(), c=state.dashboardCalendar||{month:n.getMonth(),year:n.getFullYear()}, d=new Date(c.year,c.month+Number(delta||0),1); state.dashboardCalendar={month:d.getMonth(),year:d.getFullYear()}; window.renderDashboardV851();};
  window.changeDashboardYear=function(y){const n=new Date();state.dashboardCalendar=state.dashboardCalendar||{month:n.getMonth(),year:n.getFullYear()};state.dashboardCalendar.year=Number(y)||n.getFullYear();window.renderDashboardV851();};

  async function scopedNotifications(){ const s=client(); let q=s.from('notifications').select('*').order('created_at',{ascending:false}).limit(5000); if(isAdmin())return (await q).data||[]; const rows=(await q).data||[]; const scoped=await allRows851(); const entityIds=new Set([...(scoped.orders||[]).map(o=>String(o.id)),...(scoped.tickets||[]).map(t=>String(t.id))]); return rows.filter(n=>String(n.target_user||'')===uid() || norm(n.target_role)===rnorm() || norm(n.target_role)===(rnorm().endsWith('s')?rnorm().slice(0,-1):rnorm()+'s') || (n.entity_id && entityIds.has(String(n.entity_id))) || (!n.target_user && !n.target_role && entityIds.has(String(n.entity_id||'')))); }
  window.renderNotificationsV851=async function(){ if(typeof page==='function')page('Notificaciones','Avisos automáticos, cambios de estado y auditoría operativa.'); const data=await scopedNotifications(); state.rows={...(state.rows||{}),notifications:data}; $('#content').innerHTML=`<div class="card"><div class="module-head"><div><h2>Centro de notificaciones</h2><p>${isAdmin()?'Vista global.':'Solo notificaciones asociadas a mi cuenta, tickets u órdenes.'}</p></div><div class="module-actions">${isAdmin()?'<button class="btn primary" onclick="sendInternalNotification()">Enviar notificación</button>':''}<button class="btn" onclick="exportCsv('notifications')">Exportar CSV</button><button class="btn" onclick="exportPdf('notifications')">PDF A4</button></div></div><div class="notif-list notification-list">${data.map(n=>`<div class="notif-card notification-card ${n.is_read?'read':'unread'}"><b>${E(n.title)}</b><p>${E(n.body||'')}</p><small>${fmt2(n.created_at)} · ${E(n.module||'')}</small><span class="badge">${n.is_read?'Leída':'Sin leer'}</span></div>`).join('')||'<p>Sin notificaciones para esta cuenta.</p>'}</div></div>`; };
  window.renderNotifications=window.renderNotificationsV851;
  window.updateNotificationCountV851=async function(){try{const data=await scopedNotifications(); const c=data.filter(n=>!n.is_read).length; const b=$('#notifyBtn .notif-badge-count')||$('#notifyBtn .badge')||$('#notifyBtn span'); if(b){b.textContent=c;b.hidden=!c;}}catch(_){}}; window.updateNotificationCountV850=window.updateNotificationCountV851;

  async function callRepairOrphanProfile(v){ const s=client(); const {data,error}=await s.rpc('medtuc_repair_orphan_profile',{p_email:v.email,p_full_name:v.full_name,p_role_name:v.role_name,p_office:v.office,p_phone:v.phone,p_is_active:v.is_active,p_work_days:v.work_days,p_work_start:v.work_start,p_work_end:v.work_end}); if(error)throw error; return data; }
  const oldAdminCreate=window.callAdminCreateUserV836;
  window.callAdminCreateUserV851=async function(v){ try{ if(oldAdminCreate) return await oldAdminCreate(v); }catch(e){ const msg=String(e.message||e); if(/already|registered|database error checking email|duplicate|exists|email/i.test(msg)){ return await callRepairOrphanProfile(v); } throw e; } return await callRepairOrphanProfile(v); };
  window.callAdminCreateUserV836=window.callAdminCreateUserV851;

  async function loadTasks(){const r=await client().from('task_types').select('*').eq('is_active',true).order('name');return r.data||[]}
  async function pickTechnician(taskId){ if(!taskId)return null; const r=await client().rpc('medtuc_pick_technician_for_task',{p_task_type_id:taskId}); if(r.error){console.warn(r.error.message);return null} return Array.isArray(r.data)?r.data[0]:r.data; }
  async function createOrderFromTicket(ticket, tech){ const s=client(); let st=(await s.from('service_order_statuses').select('id').ilike('name','%Pendiente%').limit(1).maybeSingle()).data; const payload={origin_ticket_id:ticket.id,status_id:st?.id||null,assigned_to:tech?.technician_id||null,technician_user_id:tech?.technician_id||null,professional_technician:tech?.full_name||null,requester_name:ticket.requester_name,requester_email:ticket.requester_email,requester_phone:ticket.requester_phone,office:ticket.office,area:ticket.area,equipment_type:ticket.equipment_type,brand:ticket.brand,model:ticket.model,accessories:ticket.accessories,fault_description:ticket.description||ticket.subject||'Ticket generado desde Soporte Ticket',priority:ticket.priority||'Media',source:'Soporte Ticket',created_by:uid(),updated_by:uid(),task_type_id:ticket.task_type_id||null,service_type:ticket.service_type||ticket.incidence_type||null,equipment_count:Number(ticket.equipment_count||1)}; const r=await s.from('service_orders').insert(payload).select('id,order_number').single(); if(r.error)throw r.error; await s.from('support_tickets').update({service_order_id:r.data.id,assigned_to:tech?.technician_id||null,technician_user_id:tech?.technician_id||null,status:'Derivado a Orden de Servicio'}).eq('id',ticket.id); await s.from('notifications').insert({title:'Nueva Orden de Servicio asignada',body:`Orden #${r.data.order_number||''} generada desde ticket ${ticket.ticket_number||''}.`,module:'orders',entity_id:r.data.id,target_user:tech?.technician_id||null,created_by:uid(),is_read:false}); return r.data; }
  const prevOpenTicket=window.openTicketFormV848||window.openTicketFormV847||window.openTicketFormV844;
  window.openTicketFormV851=async function(id=null){ if(id && prevOpenTicket)return prevOpenTicket(id); const tasks=await loadTasks(); const taskOptions=tasks.map(t=>`<option value="${E(t.id)}">${E(t.name)}${t.weight?' · peso '+E(t.weight):''}</option>`).join(''); const html=`<div class="tm-form"><div class="tm-form-header"><div class="tm-form-avatar">${(window.ico&&ico.tickets)||'🎫'}</div><p>El ticket generará una Orden de Servicio y asignará un Profesional Técnico disponible según tarea, horario y excepciones.</p></div><div class="swal-grid"><label>Solicitante<input id="t_requester_name" value="${E(state.profile?.full_name||'')}"></label><label>Email<input id="t_requester_email" value="${E(state.profile?.email||'')}"></label><label>Teléfono<input id="t_requester_phone" value="${E(state.profile?.phone||'')}"></label><label>Oficina/Repartición<input id="t_office" value="${E(state.profile?.office||'')}"></label><label>Área<input id="t_area"></label><label>Prioridad<select id="t_priority"><option>Baja</option><option selected>Media</option><option>Alta</option><option>Urgente</option></select></label><label>Tipo de tarea<select id="t_task_type_id">${taskOptions}</select></label><label>Tipo de equipo<input id="t_equipment_type"></label><label>Marca<input id="t_brand"></label><label>Modelo<input id="t_model"></label><label class="full">Asunto<input id="t_subject" placeholder="Ej. Cableado de Red"></label><label class="full">Descripción<textarea id="t_description" placeholder="Detalle técnico de la solicitud"></textarea></label></div></div>`; const {value}=await Swal.fire({title:'Nuevo Ticket de Soporte',html,width:940,showCancelButton:true,confirmButtonText:'Crear ticket y OS',customClass:{popup:'tm-modal'},preConfirm:()=>({requester_name:$('#t_requester_name').value.trim(),requester_email:$('#t_requester_email').value.trim().toLowerCase(),requester_phone:$('#t_requester_phone').value.trim(),office:$('#t_office').value.trim(),area:$('#t_area').value.trim(),subject:$('#t_subject').value.trim(),incidence_type:$('#t_task_type_id').selectedOptions[0]?.textContent?.replace(/ · peso.*$/,'')||'Soporte',description:$('#t_description').value.trim(),priority:$('#t_priority').value,task_type_id:$('#t_task_type_id').value,equipment_type:$('#t_equipment_type').value.trim(),brand:$('#t_brand').value.trim(),model:$('#t_model').value.trim(),created_by:uid(),status:'Pendiente'})}); if(!value)return; if(!value.subject||!value.description)return Swal.fire({icon:'warning',title:'Asunto y descripción son obligatorios'}); try{ const tech=await pickTechnician(value.task_type_id); if(!tech)throw new Error('No hay Profesional Técnico disponible para la tarea seleccionada según horario/excepciones. Verifique el módulo Excepciones y la asignación de tareas por técnico.'); const ins=await client().from('support_tickets').insert({...value,assigned_to:tech.technician_id,technician_user_id:tech.technician_id}).select('*').single(); if(ins.error)throw ins.error; const os=await createOrderFromTicket(ins.data,tech); await Swal.fire({icon:'success',title:'Ticket y Orden creados',html:`Asignado a <b>${E(tech.full_name)}</b><br>Orden de Servicio #${E(os.order_number||'')}`}); route('tickets'); }catch(e){Swal.fire({icon:'error',title:'No se pudo crear ticket/orden',text:e.message});} };
  const prevOpenForm=window.openForm; window.openForm=function(key,id=null){ if(String(key)==='tickets')return window.openTicketFormV851(id); return prevOpenForm?prevOpenForm(key,id):undefined; }; try{openForm=window.openForm}catch(_){ }

  const prevRoute=window.route; window.route=async function(p){ const k=String(p||'dashboard'); state.page=k; if(k==='dashboard')return window.renderDashboardV851(); if(k==='notifications')return window.renderNotificationsV851(); return prevRoute?prevRoute(p):undefined; }; try{route=window.route}catch(_){ }
  setTimeout(()=>{addCss(); if(state?.page==='dashboard')window.renderDashboardV851(); window.updateNotificationCountV851?.();},900);
})();

/* ==========================================================
   MEDTUC v8.52 FINAL HOTFIX
   - SuperAdmin: menú completo + lectura global de órdenes.
   - Referente/Usuario: scope por oficina/cuenta.
   - Configuración de ícono PWA instalable.
   - Dashboard mobile first: calendario sin desborde.
   - Header mobile: logo alineado.
   - Botón Cafecito oficial.
   ========================================================== */
(function(){
  const $ = window.$ || ((s, r=document)=>r.querySelector(s));
  const $$ = window.$$ || ((s, r=document)=>Array.from(r.querySelectorAll(s)));
  const esc = window.esc || window.E || (v=>String(v ?? '').replace(/[&<>'"]/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;',"'":'&#39;','"':'&quot;'}[m])));
  const norm = v => String(v || '').normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase().trim();
  const role = () => String(window.state?.profile?.role_name || '');
  const isSuper = () => norm(role()) === 'superadmin';
  const isAdmin = () => ['superadmin','admin'].includes(norm(role()));
  const isRef = () => norm(role()).startsWith('referente');
  const isTech = () => ['tecnico','tecnicos'].includes(norm(role()));
  const isUser = () => ['usuario','usuarios'].includes(norm(role()));
  const uid = () => String(window.state?.user?.id || '');
  const email = () => String(window.state?.profile?.email || window.state?.user?.email || '').toLowerCase();
  const office = () => norm(window.state?.profile?.office || '');
  const fmtDate = v => { try { return v ? new Date(v).toLocaleString('es-AR') : '-'; } catch { return '-'; } };
  const icon = (k) => (window.ico && (ico[k] || ico.dashboard)) || '';
  const fullModules = [
    ['dashboard','Dashboard','dashboard'],['users','Usuarios','users'],['roles','Roles y Permisos','roles'],['orders','Órdenes de Servicio','orders'],
    ['inventory','Inventario','inventory'],['loans','Gestión de Préstamos','loans'],['tickets','Soporte Ticket','tickets'],['offices','Dtos/Oficinas','offices'],
    ['exceptions','Excepciones','calendar'],['logs','Registros Historial','logs'],['notifications','Notificaciones','bell'],['profile','Mi Perfil','user'],['settings','Configuraciones','settings'],['logout','Cerrar Sesión','logout']
  ];
  function allowedModules852(){
    if(isSuper()) return fullModules.map(m=>[m[0],m[1],icon(m[2])]);
    if(isTech()) return [['dashboard','Dashboard',icon('dashboard')],['orders','Órdenes de Servicio',icon('orders')],['loans','Gestión de Préstamos',icon('loans')],['tickets','Soporte Ticket',icon('tickets')],['notifications','Notificaciones',icon('bell')],['profile','Mi Perfil',icon('user')],['settings','Configuraciones',icon('settings')],['logout','Cerrar Sesión',icon('logout')]];
    if(isRef()) return [['dashboard','Dashboard',icon('dashboard')],['tickets','Soporte Ticket',icon('tickets')],['orders','Órdenes de Servicio',icon('orders')],['notifications','Notificaciones',icon('bell')],['profile','Mi Perfil',icon('user')],['logout','Cerrar Sesión',icon('logout')]];
    return [['dashboard','Dashboard',icon('dashboard')],['loans','Gestión de Préstamos',icon('loans')],['tickets','Soporte Ticket',icon('tickets')],['notifications','Notificaciones',icon('bell')],['profile','Mi Perfil',icon('user')],['logout','Cerrar Sesión',icon('logout')]];
  }
  function inject852Css(){
    if($('#medtuc-v852-css')) return;
    const st=document.createElement('style'); st.id='medtuc-v852-css'; st.textContent=`
      .sidebar-foot .cafecito img{max-width:145px;height:auto;display:block;margin-top:7px}.sidebar-foot .cafecito{display:inline-block;border:0!important;background:transparent!important;padding:0!important}
      .stat-card{cursor:pointer}.stat-card:nth-child(2),.op-urgente{border-left:5px solid #ef4444!important}.stat-card:nth-child(3),.op-atrasada{border-left:5px solid #f97316!important}.op-pendiente{border-left:5px solid #f59e0b!important}.op-lista{border-left:5px solid #22c55e!important}.alert-grid b{background:rgba(148,163,184,.20);border-radius:10px;padding:10px 12px;display:block}.status-row{display:flex;gap:8px;justify-content:space-between;border-bottom:1px solid rgba(148,163,184,.18);padding:5px 0}.table-wrap{width:100%;overflow:auto;border:1px solid rgba(148,163,184,.22);border-radius:16px}.table-wrap table{width:100%;border-collapse:collapse}.table-wrap th,.table-wrap td{padding:12px;border-bottom:1px solid rgba(148,163,184,.14);text-align:left;vertical-align:top}.table-wrap th{font-size:12px;text-transform:uppercase;letter-spacing:.06em}.badge{display:inline-flex;align-items:center;border-radius:999px;padding:5px 9px;font-weight:800;font-size:12px;background:rgba(124,92,255,.14);color:inherit}.badge.danger{background:rgba(239,68,68,.16)}.badge.ok{background:rgba(34,197,94,.16)}
      .calendar-card{overflow:hidden}.calendar-head{display:flex;align-items:center;justify-content:space-between;gap:12px;flex-wrap:wrap}.calendar-controls{display:flex;gap:8px;align-items:center;flex-wrap:wrap}.calendar-grid{display:grid;grid-template-columns:repeat(7,minmax(0,1fr));gap:8px;min-width:0}.cal-day-name{font-weight:900;color:#9fb1d1;text-transform:uppercase;font-size:12px}.cal-cell{min-height:84px;border-radius:12px;background:rgba(148,163,184,.38);padding:8px;overflow:hidden;position:relative}.cal-cell .day-num{font-weight:900;text-align:right;display:block}.cal-event{display:block;margin-top:4px;border-radius:8px;background:#6d55c7;color:#fff;padding:5px 6px;font-size:11px;font-weight:800;line-height:1.15;white-space:normal;overflow:hidden}.mobile-brand-top{display:none}.mobile-brand-top img{object-fit:contain;display:block}.app-icon-preview{width:72px;height:72px;border-radius:18px;object-fit:contain;background:#0f172a;padding:8px;border:1px solid rgba(148,163,184,.30)}
      @media(max-width:760px){body{overflow-x:hidden}.layout{width:100%;max-width:100vw}.main{max-width:100vw;overflow-x:hidden;padding-bottom:92px}.topbar{display:flex;align-items:center;gap:8px}.mobile-brand-top{display:flex!important;width:38px;height:38px;align-items:center;justify-content:center;flex:0 0 38px;border-radius:11px;background:rgba(148,163,184,.18)}.mobile-brand-top img{max-width:31px;max-height:31px}.topbar-title{min-width:0;flex:1}.topbar-title h1{font-size:20px;line-height:1.1}.topbar-title p{display:none}.actions{gap:5px;flex:0 0 auto}.avatar-action{padding:4px!important}.avatar-action img{width:30px;height:30px}.dash-grid.cards{display:grid!important;grid-template-columns:repeat(2,minmax(0,1fr))!important;gap:10px!important}.stat-card{min-width:0!important;padding:14px!important}.dashboard-main{display:block!important}.dashboard-main .card{margin-top:12px}.calendar-card{padding:12px!important;max-width:100%;overflow:hidden}.calendar-head{align-items:flex-start}.calendar-controls{width:100%;display:grid;grid-template-columns:1fr 1fr;gap:8px}.calendar-controls .btn,.calendar-controls button,.calendar-controls select{width:100%;min-width:0}.calendar-grid{grid-template-columns:repeat(7,minmax(36px,1fr));gap:4px;width:100%;overflow:visible}.cal-day-name{font-size:10px}.cal-cell{min-height:58px;border-radius:9px;padding:4px}.cal-cell .day-num{font-size:11px}.cal-event{font-size:9px;padding:3px 4px;max-height:34px}.sidebar{display:none!important}.mobile-dock{display:flex!important}.table-wrap{font-size:12px}.table-wrap th,.table-wrap td{padding:8px}.card{max-width:100%;overflow:hidden}}
    `; document.head.appendChild(st);
  }
  function applyDynamicManifest(){
    const s=window.state?.settings||{};
    const iconUrl=s.app_icon_url || s.logo_light_url || 'assets/logo.svg';
    const name=s.institution_name ? `Ticket Manager - ${s.institution_name}` : 'Ticket Manager - Ministerio de Educación Tucumán';
    const manifest={name,short_name:'Ticket Manager',start_url:'./app.html',scope:'./',display:'standalone',background_color:s.background_color||'#08111f',theme_color:s.primary_color||'#7c5cff',icons:[{src:iconUrl,sizes:'192x192',type:'image/png',purpose:'any maskable'},{src:iconUrl,sizes:'512x512',type:'image/png',purpose:'any maskable'}]};
    try{ const blob=new Blob([JSON.stringify(manifest)],{type:'application/manifest+json'}); const url=URL.createObjectURL(blob); let l=document.querySelector('link[rel="manifest"]'); if(!l){l=document.createElement('link');l.rel='manifest';document.head.appendChild(l);} l.href=url; let ai=document.querySelector('link[rel="apple-touch-icon"]'); if(ai) ai.href=iconUrl; let fi=document.querySelector('link[rel="icon"]'); if(fi) fi.href=iconUrl; }catch(_){ }
  }
  window.renderShell=function(){
    inject852Css(); applyDynamicManifest();
    const s=window.state?.settings||{}; const logo=(localStorage.theme==='light'?s.logo_light_url:s.logo_dark_url)||s.logo_light_url||'assets/logo.svg'; const av=window.state?.profile?.avatar_url||'assets/avatar-default.svg';
    const mods=allowedModules852(); const mobileMain=mods.filter(m=>['dashboard','orders','tickets','notifications','profile'].includes(m[0])).slice(0,4);
    const themeIcon=localStorage.theme==='light'?(typeof themeMoonSvg==='function'?themeMoonSvg():'☾'):(typeof themeSunSvg==='function'?themeSunSvg():'☼');
    $('#app').innerHTML=`<div class="layout"><aside class="sidebar"><div class="brand" id="brandToggle"><img src="${esc(logo)}" onerror="this.src='assets/logo.svg'"><div><strong>${esc(s.institution_name||'Ministerio de Educación Tucumán')}</strong><small>${esc(s.institution_area||'Dirección de Informática - Área Soporte Técnico')}</small></div></div><nav class="nav">${mods.map(m=>`<button data-page="${esc(m[0])}">${m[2]||''}<span>${esc(m[1])}</span></button>`).join('')}</nav><div class="sidebar-foot"><div>TICKET & OS MANAGER © 2026</div><a class='cafecito' href='https://cafecito.app/fmgambino' rel='noopener' target='_blank'><img srcset='https://cdn.cafecito.app/imgs/buttons/button_3.png 1x, https://cdn.cafecito.app/imgs/buttons/button_3_2x.png 2x, https://cdn.cafecito.app/imgs/buttons/button_3_3.75x.png 3.75x' src='https://cdn.cafecito.app/imgs/buttons/button_3.png' alt='Invitame un café en cafecito.app' /></a></div></aside><main class="main"><section class="topbar"><div class="mobile-brand-top"><img src="${esc(logo)}" onerror="this.src='assets/logo.svg'"></div><div class="topbar-title"><h1 id="pageTitle"></h1><p id="pageDesc"></p></div><div class="actions"><button class="icon-btn desktop-only theme-svg" id="themeBtn">${themeIcon}</button><button class="icon-btn desktop-only" id="fullBtn">⛶</button><button class="icon-btn notify-trigger" id="notifyBtn">${icon('bell')}<span class="notif-badge-count" hidden>0</span></button><button class="avatar-action" id="avatarBtn"><img src="${esc(av)}" onerror="this.src='assets/avatar-default.svg'"><span class="desktop-only">${esc(window.state?.profile?.full_name||window.state?.profile?.email||'')}</span></button><button class="icon-btn logout-action" id="logoutBtn">${icon('logout')||'⇲'}</button></div></section><section id="content"></section><footer class="footer">TICKET MANAGER © 2026 Tucumán - Argentina<br>by <a href="https://github.com/fmgambino" target="_blank" rel="noopener">Ing. Fernando Gambino</a> · Todos los Derechos Registrados</footer></main><nav class="mobile-dock">${mobileMain.map(m=>`<button data-page="${esc(m[0])}">${m[2]||''}<span>${esc(m[1])}</span></button>`).join('')}<button id="mobileMoreBtn">${icon('menu')||'<svg class="icon" viewBox="0 0 24 24"><path d="M4 6h16M4 12h16M4 18h16"/></svg>'}<span>Más</span></button></nav></div>`;
    $$('.nav button,.mobile-dock button[data-page]').forEach(b=>b.onclick=()=>window.route(b.dataset.page));
    $('#themeBtn') && ($('#themeBtn').onclick=()=>{localStorage.theme=localStorage.theme==='light'?'dark':'light'; if(window.applySettings) applySettings(); window.renderShell(); window.route(window.state?.page||'dashboard');});
    $('#fullBtn') && ($('#fullBtn').onclick=()=>document.fullscreenElement?document.exitFullscreen():document.documentElement.requestFullscreen?.());
    $('#avatarBtn') && ($('#avatarBtn').onclick=()=>window.route('profile')); $('#logoutBtn') && ($('#logoutBtn').onclick=()=>window.route('logout')); $('#brandToggle') && ($('#brandToggle').onclick=()=>document.body.classList.toggle('sidebar-collapsed'));
    $('#mobileMoreBtn') && ($('#mobileMoreBtn').onclick=()=>{ const rest=mods.filter(m=>!mobileMain.some(x=>x[0]===m[0])); Swal.fire({title:'Menú',html:`<div class="mobile-menu-sheet">${rest.map(m=>`<button class="mobile-menu-item" data-page="${esc(m[0])}">${m[2]||''}<span>${esc(m[1])}</span></button>`).join('')}</div>`,showConfirmButton:false,showCloseButton:true,customClass:{popup:'tm-modal mobile-menu-modal'}}); setTimeout(()=>$$('.mobile-menu-item').forEach(b=>b.onclick=()=>{Swal.close();window.route(b.dataset.page)}),30); });
    if(window.updateNotificationCountV851) window.updateNotificationCountV851();
  };
  try{ window.renderShell(); }catch(_){ }

  function scopeOrders(rows){
    rows=rows||[]; if(isAdmin()) return rows;
    if(isTech()) return rows.filter(o=>[o.assigned_to,o.technician_user_id,o.collaborator_assigned_to,o.attended_by].map(String).includes(uid()) || norm(o.professional_technician).includes(norm(window.state?.profile?.full_name||'')));
    if(isRef()) return rows.filter(o=>norm(o.office)===office() || String(o.created_by||'')===uid() || String(o.requester_email||'').toLowerCase()===email());
    return rows.filter(o=>String(o.created_by||'')===uid() || String(o.requester_email||'').toLowerCase()===email());
  }
  function scopeTickets(rows){
    rows=rows||[]; if(isAdmin()) return rows;
    if(isTech()) return rows.filter(t=>[t.assigned_to,t.technician_user_id,t.collaborator_assigned_to,t.attended_by].map(String).includes(uid()));
    if(isRef()) return rows.filter(t=>norm(t.office)===office() || String(t.created_by||'')===uid() || String(t.requester_email||'').toLowerCase()===email());
    return rows.filter(t=>String(t.created_by||'')===uid() || String(t.requester_email||'').toLowerCase()===email());
  }
  async function fetchOrders852(){
    const r=await supa.from('service_orders').select('*,service_order_statuses(name,color)').order('received_at',{ascending:false}).limit(5000);
    if(r.error) throw r.error; return scopeOrders(r.data||[]);
  }
  async function fetchTickets852(){ const r=await supa.from('support_tickets').select('*').order('created_at',{ascending:false}).limit(5000); if(r.error)throw r.error; return scopeTickets(r.data||[]); }
  window.renderOrdersV852=async function(){
    if(window.page) page('Órdenes de Servicio','Trazabilidad técnica institucional, sin importes ni conceptos comerciales.');
    try{
      const rows=await fetchOrders852(); window.state.rows={...(window.state.rows||{}),orders:rows};
      const canManage=isAdmin()||isTech();
      $('#content').innerHTML=`<div class="card"><div class="module-head"><div><h2>Órdenes técnicas</h2><p>${isAdmin()?'SuperAdmin visualiza todas las órdenes del sistema.':isRef()?'Seguimiento de órdenes asociadas a su oficina y tickets cargados.':'Solo órdenes asociadas a su cuenta.'}</p></div><div class="module-actions"><button class="btn" onclick="exportCsv('orders')">Exportar CSV</button><button class="btn" onclick="exportPdf('orders')">PDF A4</button></div></div><input class="search" id="orderSearch852" placeholder="Buscar en órdenes..."><div class="table-wrap"><table><thead><tr><th>Orden</th><th>Ingreso</th><th>Oficina/Repartición</th><th>Profesional Técnico</th><th>Equipo</th><th>Falla / Diagnóstico</th><th>Estado</th><th>Acciones</th></tr></thead><tbody id="ordersBody852">${rows.map(orderRow).join('') || '<tr><td colspan="8">Sin órdenes asignadas.</td></tr>'}</tbody></table></div></div>`;
      $('#orderSearch852')?.addEventListener('input',e=>{const q=norm(e.target.value); $('#ordersBody852').innerHTML=(rows.filter(o=>norm(Object.values(o).join(' ')).includes(q)).map(orderRow).join('') || '<tr><td colspan="8">Sin coincidencias.</td></tr>')});
    }catch(e){ $('#content').innerHTML=`<div class="card"><h2>Error al cargar Órdenes de Servicio</h2><p>${esc(e.message||e)}</p><p>Ejecute el SQL <b>MEDTUC_V8_52_FINAL.sql</b> para actualizar las políticas RLS.</p></div>`; }
  };
  function orderRow(o){ const st=o.service_order_statuses?.name||o.status||'Sin estado'; const eq=[o.equipment_type,o.brand,o.model,o.serial_number].filter(Boolean).join(' · ')||'-'; return `<tr><td><b>OS ${esc(o.order_number||o.satmanager_order||'-')}</b></td><td>${esc(fmtDate(o.received_at||o.created_at))}</td><td>${esc(o.office||'-')}</td><td>${esc(o.professional_technician||o.technician_name||'-')}</td><td>${esc(eq)}</td><td>${esc(o.fault_description||o.diagnosis||'-')}</td><td><span class="badge">${esc(st)}</span></td><td><button class="icon-mini" onclick="viewOrder852('${esc(o.id)}')">Ver</button></td></tr>`; }
  window.viewOrder852=function(id){ const o=(window.state?.rows?.orders||[]).find(x=>String(x.id)===String(id)); if(!o)return; Swal.fire({title:`Orden ${esc(o.order_number||'')}`,html:`<div class="detail-grid"><p><b>Oficina</b><span>${esc(o.office||'-')}</span></p><p><b>Técnico</b><span>${esc(o.professional_technician||o.technician_name||'-')}</span></p><p><b>Falla</b><span>${esc(o.fault_description||'-')}</span></p><p><b>Diagnóstico</b><span>${esc(o.diagnosis||'-')}</span></p><p><b>Solución</b><span>${esc(o.solution||'-')}</span></p><p><b>Estado</b><span>${esc(o.service_order_statuses?.name||o.status||'-')}</span></p></div>`,width:760,customClass:{popup:'tm-modal'}}); };

  function finalOrder(o){ return ['terminada','entregada','cerrada','finalizada','lista p/retirar','lista','listas'].includes(norm(o.service_order_statuses?.name||o.status||'')); }
  function delayedOrder(o){ const t=new Date(o.received_at||o.created_at||Date.now()).getTime(); return Date.now()-t > 7*86400000 && !finalOrder(o); }
  function monthCalendar(orders){
    const n=new Date(); const cal=window.state.dashboardCalendar||{month:n.getMonth(),year:n.getFullYear()}; const names=['enero','febrero','marzo','abril','mayo','junio','julio','agosto','septiembre','octubre','noviembre','diciembre']; const first=new Date(cal.year,cal.month,1); const days=new Date(cal.year,cal.month+1,0).getDate(); const start=(first.getDay()+6)%7; const cells=[]; for(let i=0;i<start;i++)cells.push('<div class="cal-cell empty"></div>'); for(let d=1;d<=days;d++){ const ev=orders.filter(o=>{const x=new Date(o.received_at||o.created_at); return x.getFullYear()===cal.year && x.getMonth()===cal.month && x.getDate()===d;}).slice(0,3); cells.push(`<div class="cal-cell"><span class="day-num">${d}</span>${ev.map(o=>`<span class="cal-event">OS ${esc(o.order_number||'')} · ${esc(o.fault_description||o.office||'Orden')}</span>`).join('')}</div>`); } return `<div class="card calendar-card"><div class="calendar-head"><div><h2>Calendario de Órdenes de Servicio</h2><p>${names[cal.month]} de ${cal.year}</p></div><div class="calendar-controls"><button class="btn" onclick="changeDashboardMonth(-1)">←</button><button class="btn" onclick="changeDashboardMonth(1)">→</button><select onchange="changeDashboardYear(this.value)">${[cal.year-1,cal.year,cal.year+1].map(y=>`<option ${y===cal.year?'selected':''}>${y}</option>`).join('')}</select><button class="btn" onclick="route('orders')">Ver órdenes</button></div></div><div class="calendar-grid"><div class="cal-day-name">LUN</div><div class="cal-day-name">MAR</div><div class="cal-day-name">MIÉ</div><div class="cal-day-name">JUE</div><div class="cal-day-name">VIE</div><div class="cal-day-name">SÁB</div><div class="cal-day-name">DOM</div>${cells.join('')}</div></div>`; }
  window.renderDashboardV852=async function(){
    inject852Css(); if(window.page) page('Dashboard','Vista ejecutiva del sistema institucional con indicadores, alertas y calendario de órdenes.');
    try{ const [orders,tickets,invR]=await Promise.all([fetchOrders852(),fetchTickets852(),supa.from('inventory_items').select('*').limit(5000)]); const inv=invR.data||[]; window.state.rows={...(window.state.rows||{}),orders,tickets,inventory:inv}; const urgent=orders.filter(o=>norm(o.priority)==='urgente'); const pending=orders.filter(o=>!finalOrder(o)); const delayed=orders.filter(delayedOrder); const done=orders.filter(finalOrder); const cards=[['orders','Órdenes',orders.length,'Órdenes técnicas registradas.'],['urgent','Urgentes',urgent.length,'Prioridad crítica.'],['delayed','Atrasadas',delayed.length,'Más de 7 días abiertas.'],['tickets','Tickets',tickets.length,'Solicitudes recibidas.']]; if(isAdmin())cards.push(['inventory','Inventario',inv.length,'Activos e insumos.']); const sm={}; orders.forEach(o=>{const k=o.service_order_statuses?.name||o.status||'Sin estado'; sm[k]=(sm[k]||0)+1}); $('#content').innerHTML=`<div class="dash-grid cards">${cards.map(c=>`<div class="stat-card"><small>${esc(c[1])}</small><strong>${esc(c[2])}</strong><span>${esc(c[3])}</span></div>`).join('')}</div><div class="dashboard-main"><div class="card alert-card"><h2>Alertas operativas</h2><p>${isAdmin()?'Resumen operativo global.':'Resumen operativo según alcance de la cuenta.'}</p><div class="alert-grid"><b class="op-urgente">${urgent.length} urgentes</b><b class="op-pendiente">${pending.length} pendientes</b><b class="op-atrasada">${delayed.length} atrasadas</b><b class="op-lista">${done.length} listas/terminadas</b></div><h3>Estados</h3>${Object.entries(sm).map(([k,v])=>`<div class="status-row"><span>${esc(k)}</span><b>${v}</b></div>`).join('')||'<p>Sin órdenes para este alcance.</p>'}</div>${monthCalendar(orders)}</div>`; }catch(e){ $('#content').innerHTML=`<div class="card"><h2>Error al cargar dashboard</h2><p>${esc(e.message||e)}</p></div>`; }
  };
  window.changeDashboardMonth=function(delta){const n=new Date(), c=window.state.dashboardCalendar||{month:n.getMonth(),year:n.getFullYear()}, d=new Date(c.year,c.month+Number(delta||0),1); window.state.dashboardCalendar={month:d.getMonth(),year:d.getFullYear()}; window.renderDashboardV852();};
  window.changeDashboardYear=function(y){const n=new Date(); window.state.dashboardCalendar=window.state.dashboardCalendar||{month:n.getMonth(),year:n.getFullYear()}; window.state.dashboardCalendar.year=Number(y)||n.getFullYear(); window.renderDashboardV852();};
  window.renderDashboard=window.renderDashboardV852;

  const previousRoute = window.route;
  window.route=async function(p){
    const k=String(p||'dashboard'); window.state.page=k;
    if(k==='dashboard') return window.renderDashboardV852();
    if(k==='orders') return window.renderOrdersV852();
    if(k==='settings') return window.renderSettingsV852();
    if(k==='logout'){ await supa.auth.signOut(); location.href='index.html'; return; }
    return previousRoute ? previousRoute(k) : undefined;
  };
  try{ route=window.route; renderDashboard=window.renderDashboardV852; }catch(_){ }

  const oldSettings = window.renderSettings;
  window.renderSettingsV852=async function(){
    if(!isAdmin() && !isSuper()) return oldSettings ? oldSettings() : Swal.fire({icon:'warning',title:'Acceso restringido'});
    inject852Css(); applyDynamicManifest(); if(window.page) page('Configuraciones','Parámetros institucionales, logos, Telegram, paleta visual e ícono instalable.');
    const s=window.state.settings||{}; const iconUrl=s.app_icon_url||s.logo_light_url||'assets/logo.svg';
    $('#content').innerHTML=`<div class="grid two"><div class="card"><div class="module-head"><h2>Configuraciones generales</h2><button class="btn primary" onclick="saveSettingsV852()">Guardar ahora</button></div><label>Nombre de la institución<input id="set_institution_name" value="${esc(s.institution_name||'')}"></label><label>Subtítulo / Área<input id="set_institution_area" value="${esc(s.institution_area||'')}"></label><label>Logo modo oscuro URL<input id="set_logo_dark_url" value="${esc(s.logo_dark_url||'')}"></label><label>Logo modo claro URL<input id="set_logo_light_url" value="${esc(s.logo_light_url||'')}"></label><label>Ícono de aplicación instalada / PWA URL<input id="set_app_icon_url" value="${esc(iconUrl)}"><small>Usar imagen cuadrada PNG/WebP recomendada 512×512.</small></label><img class="app-icon-preview" src="${esc(iconUrl)}" onerror="this.src='assets/logo.svg'"></div><div class="card"><h2>Canal de Telegram</h2><label>Enviar copias a Telegram<select id="set_telegram_enabled"><option value="false" ${!s.telegram_enabled?'selected':''}>No</option><option value="true" ${s.telegram_enabled?'selected':''}>Sí</option></select></label><label>Bot Token<input id="set_telegram_bot_token" value="${esc(s.telegram_bot_token||'')}"></label><label>Chat ID / Canal<input id="set_telegram_chat_id" value="${esc(s.telegram_chat_id||'')}"></label></div></div><div class="card"><div class="module-head"><h2>Paleta visual de la PWA</h2><button class="btn primary" onclick="saveSettingsV852()">Guardar paleta</button></div><div class="grid"><label>Primario<input type="color" id="set_primary_color" value="${esc(s.primary_color||'#7c5cff')}"></label><label>Secundario<input type="color" id="set_secondary_color" value="${esc(s.secondary_color||'#19d3da')}"></label><label>Acento<input type="color" id="set_accent_color" value="${esc(s.accent_color||'#22c55e')}"></label><label>Fondo<input type="color" id="set_background_color" value="${esc(s.background_color||'#08111f')}"></label><label>Tarjetas<input type="color" id="set_card_color" value="${esc(s.card_color||'#101b31')}"></label></div></div>`;
  };
  window.renderSettings=window.renderSettingsV852;
  window.saveSettingsV852=async function(){
    const payload={id:1,institution_name:$('#set_institution_name')?.value||'',institution_area:$('#set_institution_area')?.value||'',logo_dark_url:$('#set_logo_dark_url')?.value||'',logo_light_url:$('#set_logo_light_url')?.value||'',app_icon_url:$('#set_app_icon_url')?.value||'',telegram_enabled:$('#set_telegram_enabled')?.value==='true',telegram_bot_token:$('#set_telegram_bot_token')?.value||null,telegram_chat_id:$('#set_telegram_chat_id')?.value||null,primary_color:$('#set_primary_color')?.value||'#7c5cff',secondary_color:$('#set_secondary_color')?.value||'#19d3da',accent_color:$('#set_accent_color')?.value||'#22c55e',background_color:$('#set_background_color')?.value||'#08111f',card_color:$('#set_card_color')?.value||'#101b31',updated_at:new Date().toISOString()};
    const r=await supa.from('app_settings').upsert(payload,{onConflict:'id'}).select('*').single(); if(r.error) return Swal.fire({icon:'error',title:'No se pudo guardar',text:r.error.message}); window.state.settings=r.data; if(window.applySettings) applySettings(); applyDynamicManifest(); window.renderShell(); await window.route('settings'); Swal.fire({icon:'success',title:'Configuración guardada'});
  };
  setTimeout(()=>{inject852Css(); applyDynamicManifest(); if(window.state?.page==='dashboard') window.renderDashboardV852();},700);
})();
