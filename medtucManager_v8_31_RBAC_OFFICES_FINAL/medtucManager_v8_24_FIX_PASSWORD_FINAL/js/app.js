...
const modules = [
  ...
  ['notifications','Notificaciones',ico.bell], ['offices','Dtos/Oficinas/Reparticiones',ico.offices], ['profile','Mi Perfil',ico.profile], ['settings','Configuraciones',ico.settings], ['logout','Cerrar Sesión',ico.logout]
];
const crud = {
...
(function(){
  ...
  const MODULE_DEFS_V830 = [
    ['dashboard','Dashboard'],['users','Usuarios'],['roles','Roles y Permisos'],['orders','Órdenes de Servicio'],['inventory','Inventario'],['loans','Gestión de Préstamos'],['tickets','Soporte Ticket'],['offices','Dtos/Oficinas/Reparticiones'],['notifications','Notificaciones'],['profile','Mi Perfil'],['settings','Configuraciones']
  ];
  ...
  window.renderOfficesV830 = async function(){
    page('Dtos/Oficinas/Reparticiones','Departamentos, reparticiones, oficinas, dependencias y habitaciones centralizadas para todos los módulos.');
  ...
  window.viewOfficeV830 = function(id){ const r=(state.rows.offices||[]).find(x=>String(x.id)===String(id)); if(!r)return; Swal.fire({title:'Detalle oficina/repartición',html:`<div class="detail-grid"><p><b>Departamento</b><span>${safeHtml(r.department||'-')}</span></p><p><b>Repartición</b><span>${safeHtml(r.repartition||'-')}</span></p><p><b>Oficina</b><span>${safeHtml(r.office_name||r.name||'-')}</span></p><p><b>Dependencia</b><span>${safeHtml(r.dependency||'-')}</span></p><p><b>Habitación</b><span>${safeHtml(r.room||'-')}</span></p></div>`,customClass:{popup:'tm-modal'},width:720}); };
  ...
  if(!modules.some(m=>m[0]==='offices')) modules.splice(Math.max(0, modules.findIndex(m=>m[0]==='notifications')),0,['offices','Dtos/Oficinas/Reparticiones',ico.settings]);
  crud.offices={table:'offices',title:'Dtos/Oficinas/Reparticiones',desc:'Departamentos, reparticiones, oficinas, dependencias y habitaciones.',fields:['department','repartition','office_name','dependency','room','is_active'],labels:['Departamento','Repartición','Oficina','Dependencia','Habitación','Activo'],select:'*'};
...
function rbModuleLabel(m){
  return ({dashboard:'Dashboard',users:'Usuarios',roles:'Roles y Permisos',orders:'Órdenes de Servicio',inventory:'Inventario',loans:'Gestión de Préstamos',tickets:'Soporte Ticket',notifications:'Notificaciones',offices:'Dtos/Oficinas/Reparticiones',settings:'Configuraciones',profile:'Mi Perfil'})[m]||m;
}
...
inputHtml = function(id,label,value,type='text'){
  ...
  if (['office','location','physical_location'].includes(id)){
    const list=state.offices||[];
    return `<label>${esc(label)}<select id="f_${id}"><option value="">Sin seleccionar</option>${list.map(o=>`<option value="${esc(o.name||o.office_name||'')}" ${val===(o.name||o.office_name)?'selected':''}>${esc(o.name||o.office_name||'')}</option>`).join('')}</select></label>`;
  }
...
window.openForm = async function(key,id=null){
  ...
  if(key==='offices') { state.offices = state.offices || await fetchOffices(); }
