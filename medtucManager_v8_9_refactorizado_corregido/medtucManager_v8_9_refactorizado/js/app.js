...
function cleanLegacyText(v){ return String(v||'').replace(/Profesional técnico:\s*/gi,'Profesional técnico: ').replace(/\s*\|?\s*(Costo mano de obra original|Entrega original):\s*0(?:\.0)?/gi,'').trim(); }
...
window.printOrder = (id,type) => {
  ...
  const body = type==='entrega' ? (o.solution || o.technical_report || o.observations || o.fault_description) : o.fault_description;
  const egresoExtra = type==='entrega' ? `<div class="box"><b>Detalle de egreso:</b><br><b>Informe técnico:</b> ${esc(cleanLegacyText(o.technical_report||'-'))}<br><b>Solución / Observaciones:</b> ${esc(cleanLegacyText(o.solution||o.observations||'-'))}<br><b>Profesional técnico:</b> ${esc(cleanLegacyText(extractTechFromOrder(o)||o.technician_name||'-'))}</div>` : '';
  const copy=(label)=>`<section class="copy"><div class="rotulo"><div class="logoBox"><img src="${logo}"></div><div><h1>Dirección de Informática - Área Soporte Técnico</h1><b>${title}</b><br><small>${label}</small><br><small>Fecha y hora de impresión: ${new Date().toLocaleString('es-AR')}</small></div></div><div class="box"><b>Orden:</b> ${esc(o.satmanager_order||o.order_number)} &nbsp; <b>Estado:</b> ${esc(o.service_order_statuses?.name||'')}<br><b>Ingreso original:</b> ${fmt(o.received_at)}<br><b>Profesional Técnico:</b> ${esc(cleanLegacyText(extractTechFromOrder(o)||'-'))}<br><b>Solicitante:</b> ${esc(o.requester_name||'')}<br><b>Oficina/Repartición:</b> ${esc(o.office||'')}<br><b>Teléfono:</b> ${esc(o.requester_phone||'')}</div><div class="box"><b>Equipo:</b> ${esc([o.equipment_type,o.brand,o.model].filter(Boolean).join(' · '))}<br><b>N° serie:</b> ${esc(o.serial_number||'')}<br><b>Accesorios/Insumos:</b> ${esc(o.accessories||'')}</div><div class="box"><b>Falla / Informe:</b><br>${esc(cleanLegacyText(body||''))}</div>${egresoExtra}<p class="firmas">Firma solicitante: ____________________________ &nbsp;&nbsp; Firma técnico: ____________________________</p></section>`;
  const html=`<html><head><title>${title}</title><style>@page{size:A4;margin:11mm}body{font-family:Arial;margin:0;color:#111}.copy{border-bottom:2px dashed #777;padding:0 0 12px;margin-bottom:14px}.rotulo{display:flex;gap:16px;align-items:center;text-align:left;border-bottom:2px solid #111;padding-bottom:10px;margin-bottom:12px}.logoBox{width:170px;min-height:54px;border:1px solid #ddd;border-radius:8px;display:flex;align-items:center;justify-content:center;padding:8px}.logoBox img{max-width:155px;max-height:42px;object-fit:contain}.box{border:1px solid #222;padding:9px;margin:9px 0;font-size:12px;line-height:1.5}h1{font-size:18px;margin:0 0 3px}.firmas{margin-top:24px;font-size:12px}</style></head><body>${copy(`Copia para la Oficina/Repartición: ${esc(o.office||'-')}`)}${copy('Copia para Dirección de Informática - Área Soporte Técnico')}</body></html>`;
...
function renderShell(){
  ...
  $('#fullBtn')?.addEventListener('click', () => document.fullscreenElement ? document.exitFullscreen() : document.documentElement.requestFullscreen());
  $('#notifyBtn').onclick = () => route('notifications');
  $('#notifyBtn').onmouseenter = showNotifyPreview || showNotificationPreview;
  $('#avatarBtn').onclick = changeAvatar;
  $('#logoutBtn').onclick = () => route('logout');
  loadNotificationCount();
}
