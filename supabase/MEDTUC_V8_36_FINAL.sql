...
-- Hotfix integral: RBAC, Excepciones, Horarios laborales,
-- préstamos, notificaciones y soporte ticket.
...
alter table public.support_tickets add column if not exists task_type_id uuid;

do $$ begin
  if not exists (
    select 1 from information_schema.table_constraints
    where table_schema='public' and table_name='support_tickets'
      and constraint_name='support_tickets_task_type_id_fkey'
  ) then
    alter table public.support_tickets add constraint support_tickets_task_type_id_fkey foreign key (task_type_id) references public.task_types(id);
  end if;
...
alter table public.service_orders add column if not exists task_type_id uuid;

do $$ begin
  if not exists (
    select 1 from information_schema.table_constraints
    where table_schema='public' and table_name='service_orders'
      and constraint_name='service_orders_task_type_id_fkey'
  ) then
    alter table public.service_orders add constraint service_orders_task_type_id_fkey foreign key (task_type_id) references public.task_types(id);
  end if;
exception when undefined_table then null; end $$;
...
('SuperAdmin','Acceso total',true),('Admin','Administración general',true),('Técnicos','Perfil técnico',true),('Usuarios','Usuario general',true),('Referentes','Referentes solicitantes',true)
...
-- Asociar todos los técnicos a todas las tareas por defecto si aún no tienen asignaciones.
...
-- Permisos base. SuperAdmin todo. Técnicos operativo. Usuarios/Referentes solo lo propio.
...
select r.name, m.module_key, true,true,true,true,true,true
from public.roles r
cross join (values ('dashboard'),('users'),('roles'),('orders'),('inventory'),('loans'),('tickets'),('notifications'),('offices'),('exceptions'),('settings')) as m(module_key)
...
('Admin','dashboard',true,false,false,false,false,false),('Admin','users',true,true,true,false,false,true),('Admin','orders',true,true,true,false,true,true),('Admin','inventory',true,true,true,false,true,true),('Admin','loans',true,true,true,false,false,true),('Admin','tickets',true,true,true,false,true,true),('Admin','notifications',true,true,true,false,false,true),('Admin','offices',true,true,true,false,true,true),('Admin','roles',true,false,false,false,false,false),
...
can_view=excluded.can_view, can_create=excluded.can_create, can_edit=excluded.can_edit, can_delete=excluded.can_delete, can_import=excluded.can_import, can_export=excluded.can_export, updated_at=now();

-- -----------------------------
-- RLS/grants amplios para PWA institucional
-- -----------------------------
...
-- Desactivar RLS en tablas operativas si existiera bloqueo de policies previas.
do $$
declare t text;
begin
  foreach t in array array['profiles','roles','permissions','role_permissions','role_module_permissions','inventory_items','loans','notifications','support_tickets','service_orders','service_order_statuses','offices','task_types','technician_task_types','technician_exceptions','technician_work_schedules'] loop
...
-- -----------------------------
-- Función contador notificaciones
-- -----------------------------
...
select 'V8_36_OK' as status,
  ...
  (select count(*) from public.role_module_permissions) as role_module_permissions;
