...
(function(){
  ...
  function css(){
    ...
    const st=document.createElement('style'); st.id='v841css'; st.textContent=`
      .tm-modal.dashboard-modal-v841{width:min(1160px,96vw)!important;max-width:min(1160px,96vw)!important;padding:26px 28px!important;border-radius:24px!important;}
      ...
      .dash-popup-v841 table{width:100%;min-width:880px;border-collapse:collapse;table-layout:fixed;}
      ...
      .dash-popup-v841 td{font-size:15px;color:#eef6ff;overflow-wrap:break-word;word-break:normal;}
      .dash-popup-v841 .num{width:78px;white-space:nowrap!important;word-break:keep-all!important;overflow-wrap:normal!important;font-weight:900;}
      .dash-popup-v841 .email{width:230px;}
      .dash-popup-v841 .office{width:180px;}
      .dash-popup-v841 .tech{width:145px;}
      .dash-popup-v841 .wide{width:auto;}
      .dash-popup-v841 .stock{width:90px;white-space:nowrap!important;}
      .dash-popup-v841 .actions{width:118px;text-align:center;}
      .dash-popup-v841 td.actions{display:flex;gap:8px;justify-content:center;align-items:center;white-space:nowrap;}
      .dash-popup-v841 .icon-mini{width:34px;height:34px;display:inline-flex;align-items:center;justify-content:center;}
  ...
  function btns(key,r){
    const id=E(r.id||'');
    if(key==='orders') return `<button class="icon-mini" title="Ver" onclick="Swal.close();viewOrder('${id}')">${ico.view}</button><button class="icon-mini" title="Editar" onclick="Swal.close();route('orders');setTimeout(()=>openOrder('${id}'),350)">${ico.edit}</button><button class="icon-mini danger" title="Eliminar" onclick="Swal.close();deleteOrder('${id}')">${ico.trash}</button>`;
    return `<button class="icon-mini" title="Ver" onclick="viewRecord('${key}','${id}')">${ico.view}</button><button class="icon-mini" title="Editar" onclick="Swal.close();route('${key}');setTimeout(()=>openForm('${key}','${id}'),350)">${ico.edit}</button><button class="icon-mini danger" title="Eliminar" onclick="Swal.close();deleteRow('${key}','${id}')">${ico.trash}</button>`;
  }
  function cell(r,k){
